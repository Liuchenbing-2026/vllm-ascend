...
## 3 A2 Deployment Configurations

### 3.1 Single-Node PD Colocated Deployment

This configuration deploys Prefill and Decode colocated on one A2 node with TP8.
...
exec vllm serve /mnt/share/weight/DeepSeek-V4-Flash-0731-w8a8 \
    --host 0.0.0.0 \
    --port "${server_port}" \
...
- `VLLM_PREFIX_CACHE_RETENTION_INTERVAL=4096`: recommended A2 Prefill prefix-cache retention interval when `--block-size` is `32`.
...
- `--no-disable-hybrid-kv-cache-manager`: keeps the hybrid KV cache manager enabled.
- `MooncakeHybridConnector`: transfers KV cache between Prefill and Decode nodes.
- `enable_flashcomm1`: enables FlashComm1 communication optimization.
- `multistream_overlap_shared_expert`: overlaps shared-expert computation on Decode nodes.
- `VLLM_PREFIX_CACHE_RETENTION_INTERVAL`: Controls the retention interval, in tokens, for prefix-cache checkpoints of hybrid attention layers. The value must be a non-negative multiple of `--block-size`.
