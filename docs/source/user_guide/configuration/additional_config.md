...
| `tokenizer_cache_gb`                 | int  | `0`     | Size of the renderer-side incremental (segment-level) tokenizer cache, in GiB, per API server process. `0` disables it. It only pays off at very high PrefixCache hit rates, where tokenization — still O(full prompt) on every turn — dominates the frontend cost. The cache lives in host memory and never consumes NPU memory; with `--api-server-count N` the footprint is N times this value. |
