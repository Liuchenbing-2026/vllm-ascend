...
Unlike DeepSeek, Gemma4 ships its MTP head as a **separate assistant checkpoint**, so the draft model must be passed
explicitly via `model` in `speculative_config`. The assistant shares the target's KV cache through cross-layer KV
sharing, so it adds almost no KV memory. `num_speculative_tokens` is fixed to `1`.
...
- Online inference
    ...
    ```shell
    vllm serve google/gemma-4-E2B-it     --port 20004     --tensor-parallel-size 1     --max-model-len 4096     --limit-mm-per-prompt '{"image":0,"audio":0}'     --compilation-config '{"cudagraph_mode": "FULL_DECODE_ONLY"}'     --speculative-config '{"method":"mtp", "model":"google/gemma-4-E2B-it-assistant", "num_speculative_tokens":1}'
    ```

> [!NOTE]
> The target and the assistant must be paired: the assistant's `backbone_hidden_size` has to match the target's
> `hidden_size`. Pairing `gemma-4-E2B-it` with, say, the E4B assistant will fail at load time.
>
> The target is multimodal while the assistant is text-only. This is expected — the proposer probes the draft model
> and logs `Draft model does not support multimodal inputs, falling back to text-only mode`.
