...
def get_cann_megamoe_dummy_token_capacity(num_experts: int, num_topk: int) -> int:
    """Reserve enough global dummy rows to route to every expert once."""
    if num_topk < 1:
...
def get_hccl_config_for_pg_options(group_name: str) -> dict | None:
    ...
    if group_name and "mc2" in group_name:
        ...
        # Other MC2 operators partition communication space from HCCL_BUFFSIZE;
        # setting process-group options for those paths causes misalignment.
        return None
    hccl_config_map = {
...
def should_skip_allreduce_across_dp_group(vllm_config: VllmConfig, is_draft_model: bool = False) -> bool:
    ...
    def needs_mc2(n: int) -> bool:
        return select_moe_comm_method(n, vllm_config, is_draft_model) in {
            MoECommType.MC2,
