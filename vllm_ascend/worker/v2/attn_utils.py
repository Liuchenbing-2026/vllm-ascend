...
def get_kv_cache_spec(vllm_config: VllmConfig) -> dict[str, KVCacheSpec]:
    ...
    if mamba_specs:
        ...
        for layer_name, spec in mamba_specs.items():
            if spec.page_size_bytes < common_page_size:
                mamba_specs[layer_name] = replace(
                    spec, page_size_padded=common_page_size
                )
...
def _allocate_kv_cache(
    ...
    for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
        ...
        # Hybrid Attention/Mamba groups share one scheduler allocation. Expose
        # the same raw storage to every participating layer; reshape below
        # creates the state and K/V views expected by the kernels.
        contains_mamba = any(
...
def _reshape_mamba_kv_cache(
    ...
    for shape, dtype in zip(kv_cache_spec.shapes, kv_cache_spec.dtypes):
        ...
        end_idx = (
            start_idx
            + torch.empty(target_shape, device="meta").numel()
...
def _reshape_kv_cache_v2(
    ...
    for group in attn_groups:
        ...
        if group_spec.storage_block_size != group_spec.block_size:
            kernel_block_size = group_spec.storage_block_size
        else:
        ...
        for layer_name in group.layer_names:
            ...
            if not isinstance(kv_cache_spec, AttentionSpec):
                raise TypeError(
                    f"Unsupported KV cache spec: {type(kv_cache_spec).__name__}."
                )
            ...
            if isinstance(raw_cache, tuple):
                ...
                if kv_start < 0:
                    raise ValueError(
                        f"Attention cache views exceed the allocation for {layer_name}."
                    )
