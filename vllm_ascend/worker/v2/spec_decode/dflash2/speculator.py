...
#
# DFlash2Speculator and its Triton kernels are adapted from vLLM #52816
# (vllm/v1/worker/gpu/spec_decode/dflash2/speculator.py, plus
# gumbel_noised_argmax from vllm/v1/worker/gpu/sample/gumbel.py). The supported
# vLLM 0.26 pin contains neither.
...
class DFlash2Speculator(DFlashSpeculator):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        if self.draft_logits is not None:
            # vLLM 0.26 allocates the cached proposal distribution as fp32 zeros
            # and has no draft_logits_spec hook, so apply the DFlash2 spec here.
            dtype, fill = self.draft_logits_spec(vllm_config)
            self.draft_logits = self.draft_logits.to(dtype).fill_(fill)
...
class AscendDFlash2Speculator(DFlash2Speculator, AscendDFlashSpeculator):
    """Run the DFlash2 selector through the Ascend DFlash path.
