...
class DFlash2Qwen3Model(DFlashQwen3Model):
    ...
    def __init__(
        ...
        *,
        ...
        # Initialize the compile-wrapper state inherited from the decorated
        # v0.23 base without the final prefix. The base temporarily constructs
        # hard-coded DFlash1 layers, which are replaced below; giving those
        # temporary layers the final prefix would register duplicate attention
        # names when the DFlash2 layers are constructed.
        super().__init__()
        ...
        # The selector has a separate compiled input signature from the draft
        # backbone. Give it a distinct cache namespace so both compiled
        # regions cannot reuse the same artifact during one startup.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        """Load DFlash2 weights with an optional outer ``model.`` prefix.
        ...
        loader indexes parameters relative to this model.
        """

        def relative_weights() -> Iterable[tuple[str, torch.Tensor]]:
            for name, weight in weights:
                yield name.removeprefix("model."), weight

        return super().load_weights(relative_weights())
