...
def _make_draft_vllm_config(
    ...
    # Ascend ModelSlim keeps quant metadata in quant_model_description.json, not
    # in config.json's quantization_config, so get_quant_config falls through to
    # the hf_overrides branch. The draft ModelConfig carries a callable
    # hf_overrides (compose_draft_hf_overrides) which that branch rejects with
    # "hf_overrides must be a dict". Normalize to {} so the draft follows the
    # verifier's file-based ModelSlim path (same fix as step3p5).
    ...
    if draft_quant_config is not None:
        # ModelSlim stores the MTP entries under the checkpoint's own layer index
        # (``mtp.layers.0.*``), but the draft decoder layers are built with the
        # prefix ``mtp.layers.<num_hidden_layers + idx>``. Mirror the description
        # keys to the runtime index so quant lookups (e.g.
        # is_layer_skipped_ascend for the fused qkv_proj) resolve.
        quant_description = getattr(draft_quant_config, "quant_description", None)
        if isinstance(quant_description, dict):
            mirrored = {}
            for key, value in quant_description.items():
                if not key.startswith("mtp.layers."):
                    continue
                shifted_key = re.sub(
                    r"(?<=mtp\.layers\.)\d+",
                    lambda m: str(mtp_start_layer_idx + int(m.group(0))),
                ...
                if shifted_key != key and shifted_key not in quant_description:
                    mirrored[shifted_key] = value
            if mirrored:
                quant_description.update(mirrored)
