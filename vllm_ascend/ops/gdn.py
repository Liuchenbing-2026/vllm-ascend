        ...
        for param, handle, event in zip(
            ...
            if run_mode == 1 and attn_metadata is not None:
                ...
                if branch == "spec" and meta.spec_sequence_masks is not None:
                    ...
                    # replay's batch has no runtime non-spec decode. That happens
                    # when spec-decode (MTP) sequences are present: the builder
                    # folds the plain decodes into prefills and sets num_decodes
                    # to 0, so `non_spec_decode_fallback_meta` is None (bug.md
                    # Bug3). Fetching host args off the missing meta raises
                    # "Expected attn_metadata.non_spec_decode_fallback_meta..."
                    # and kills the engine on req1. Make the task a state no-op
