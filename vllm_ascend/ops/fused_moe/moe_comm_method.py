...
class FusedMC2CommImpl(MoECommMethod):
    ...
    def _apply_cann_mega_moe(
        ...
        # ``_weight_type`` (ACL INT4/INT8 enum) is intentionally left UNUSED: the
        # CANN mega_moe interface doc marks ``weight1_type``/``weight2_type`` as
        # 预留参数 ("使用默认值即可" — reserved, keep the default None). The op
        ...
        # int8 weights as plain INT8), exactly like the已验证的 dispatch_ffn_combine
        # path, whose C++ signature carries no weight_type at all. Passing a
        # non-default weight1_type/weight2_type made the op's tiling mis-derive N
        # (A3 W4A8: it read l1's packed last dim 512 as N/2 instead of unpacking
        # to the real intermediate 2048), triggering the weight2 dim0 check.
