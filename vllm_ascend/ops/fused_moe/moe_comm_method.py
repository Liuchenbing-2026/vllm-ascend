...
def _append_cann_megamoe_dummy_tokens(
    ...
    # A8W8 dispatch derives a per-token scale from max(abs(x)). Sentinels
    # therefore need nonzero activations and normalized router weights.
...
class FusedMC2CommImpl(MoECommMethod):
    ...
    def _load_cann_megamoe_ops(self):
        if self._cann_megamoe_ops is None:
            try:
                ...
                get_buffer_size = getattr(module, "get_mega_moe_ccl_buffer_size", None)
                if get_buffer_size is None:
                    mega_moe_module = importlib.import_module(f"{_CANN_MEGAMOE_MODULE_NAME}.mega_moe")
                    get_buffer_size = mega_moe_module._get_mega_moe_ccl_buffer_size
    ...
    def _apply_cann_megamoe(
        ...
        # A2 does not support x_active_mask. DP steps with idle or uneven
        # ranks are selected onto standard MC2 before entering this path.
        x_active_mask = None
