...
def _draft_is_dspark(speculative_config) -> bool:
    from vllm_ascend.patch.platform.patch_speculative_config import (
        is_dspark_speculative_config,
    )

    return is_dspark_speculative_config(speculative_config)
...
def get_spec_decode_method(method, vllm_config, device, runner):
    if method == "ngram":
        ...
        if speculative_config is not None and _draft_is_dspark(speculative_config):
            # DSpark self-drafting checkpoints are detected as mtp by the core
            # (v0.23.0 has no dspark method literal); route to the DSpark
            # semi-autoregressive proposer.
