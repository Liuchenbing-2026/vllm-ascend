...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    # Declared by SpecDecodeBaseProposer.__init__; annotated here so mypy can
    # type the read in load_model() (the base class is behind a skipped import).
    supports_mm_inputs: bool

    # Proposers whose draft layers span several KV-cache groups with distinct
    # block tables opt in; every other proposer keeps the single-buffer
    # behaviour so its captured slot-mapping address is the one written at
    # runtime.
    uses_per_group_kv_groups: bool = False
        ...
        with self.runner.synchronize_input_prep():
            if aclgraph_runtime_mode == CUDAGraphMode.FULL and len(self.runner.attn_groups) > 0:
                ...
                capture_block_tables = (
                    ...
                    if self.uses_per_group_kv_groups
                    else None
                )
    ...
    def attn_update_stack_num_spec_norm(
        ...
        per_group_block_tables = self._per_group_block_tables if self.uses_per_group_kv_groups else None
        group_id = attn_group.kv_cache_group_id
        if per_group_block_tables is not None and group_id in per_group_block_tables:
