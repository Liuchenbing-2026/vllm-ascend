...
class AscendGemma4Proposer(Gemma4Proposer, AscendSpecDecodeBaseProposer):
    ...
    # Draft layers span a sliding-window group and a full-attention group with
    # different head dimensions, so each needs its own block table and
    # slot-mapping buffer.
    uses_per_group_kv_groups: bool = True

    def _setup_centroids_cuda_graphs(self) -> None:
        # torch.cuda.CUDAGraph() raises "Tried to instantiate dummy base class
        # CUDAGraph" on Ascend. Leaving _centroids_sizes empty makes
        # Gemma4Proposer._greedy_sample fall through to the ordinary path, and
        # compute_logits already applies masked_embedding.
        logger.info("Gemma4 MTP: centroids graph capture is skipped on Ascend NPU.")
