...
def _cann_megamoe_supported_by_config(vllm_config: VllmConfig) -> bool:
    ...
    if moe_intermediate_size is None:
        return False
    if os.environ.get("QWEN38_MEGAMOE_RELAX", "0") == "1":
        # Experiment switch (off by default, so the shipping selection logic is
        # untouched).  Qwen3.8-Flash-Next has moe_intermediate_size = 640, which
        # fails both the lower bound and the 512-multiple rule above, so the
        # model is routed to dispatch_ffn_combine -- 28.4 % of device time in
        # the hc3 profile (op_statistic.csv: DispatchFFNCombineBF16, 2280.8 ms
        # of 8037.8 ms over 5049 calls).  The range check is the *only* thing
        # standing between the model and the fused MegaMoe kernel, and the
        # comment above says the constraint is a CANN 9.1.0 tiling artefact
        # that 9.2.0 lifts.  This switch lets the same container answer the
        # question empirically: every other precondition (A3, fused-MC2 on,
        # expert parallelism, EP group size, hidden size, quant type) is still
        # enforced, and the teacher-forcing gate decides whether the result is
        # numerically usable before any tok/s is quoted.
        logger.warning(
            "QWEN38_MEGAMOE_RELAX=1: bypassing the CANN 9.1.0 "
            "moe_intermediate_size bounds (got %s) for the MegaMoe experiment",
            moe_intermediate_size,
        )
    elif moe_intermediate_size < 1024 or moe_intermediate_size > 3072 or moe_intermediate_size % 512 != 0:
        return False
