...
# Make the shared-experts expand-slice fallback opaque to Dynamo fullgraph.
# The Python body intentionally keeps the existing gather + mul + sum logic,
# but Dynamo only sees a mutating custom op instead of tracing its size nodes.
...
def _moe_lora_bmm_expand_slice_op(
    ...
    # The shrink buffer can be padded to the rank bucket while lora_b only carries
    # the active rank. Slice x, not gathered, to preserve the loaded weight layout.
    ...
    # profile_run / dummy_run can hand us empty tensors; downstream NPU ops trip
    # 161002 / index errors in that case.
    ...
    # aclnnMatmul / aclnnBatchMatMul 161002 path on NPU.
...
class PunicaWrapperNPU(PunicaWrapperBase):
    ...
    def _bmm_expand_slice(
        ...
        This path replaces both sgmv_expand_slice and bgmv_expand_slice under
    ...
    def moe_lora_align_block_size(
        ...
        token_lora_mapping = self._token_lora_indices[:num_tokens]
    ...
    def add_lora_fused_moe(
        ...
        * ``ascendc``         - reserved for a future fused AscendC kernel
        ...
        if token_lora_mapping is None:
            token_lora_mapping = self._token_lora_indices[: x.size(0)]
