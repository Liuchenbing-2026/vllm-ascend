...
class PunicaWrapperNPU(PunicaWrapperBase):
    ...
    def add_lora_fused_moe(
        ...
        * ``ascendc``         - reserved for a future fused AscendC kernel
        ...
        # MoE-LoRA on the eager / PIECEWISE route always uses the bgmv
        # combined-index kernel (multi-adapter). The FULL-decode grouped path
        # is selected upstream in lora/fused_moe.py and never reaches here.
        kernel = "bgmv"
    ...
    def _add_lora_fused_moe_bgmv(
        ...
        for slice_idx in range(len(lora_a_stacked)):
            ...
            if os.environ.get("MOE_LORA_DBG") != "skipbgmv":
