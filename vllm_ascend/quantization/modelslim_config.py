...
_DEEPSEEK_V4_DSPARK_MTP_LAYER_RE = re.compile(r"^model\.layers\.(\d+)\.(.+)$")
_DEEPSEEK_V4_DSPARK_QUANT_KEY_RE = re.compile(r"^(?:model\.)?mtp\.(\d+)\.(.+)$")


def _canonicalize_deepseek_v4_dspark_quant_key(name: str) -> str | None:
    """Return the vLLM quantization key for a ModelSlim DSpark tensor.

    ModelSlim checkpoint tensors use ``mtp.<stage>`` and the original DSpark
    module names.  The runtime draft model is expanded after the target layers
    and uses vLLM's DeepSeek names.  This function only translates complete
    path segments, so unrelated names containing strings such as ``attn`` or
    ``w1`` are never rewritten accidentally.
    """
    match = _DEEPSEEK_V4_DSPARK_QUANT_KEY_RE.fullmatch(name)
    if match is None:
        return None

    stage_idx, remainder = match.groups()
    ...
    if segments == ["mlp", "gate", "bias"]:
        segments[-1] = "e_score_correction_bias"

    return f"model.mtp.{stage_idx}." + ".".join(segments)


def _add_deepseek_v4_dspark_quant_aliases(quant_description: dict[str, Any]) -> None:
    """Add canonical aliases without changing checkpoint tensor names.

    A conflicting explicit canonical entry is rejected.  Silently preferring
    either entry could select a different quantization method than the tensor
    that is actually loaded, which is unsafe for speculative decoding.
    """
    aliases: dict[str, Any] = {}
    for name, quant_type in list(quant_description.items()):
        canonical_name = _canonicalize_deepseek_v4_dspark_quant_key(name)
        if canonical_name is None or canonical_name == name:
            continue
        existing = quant_description.get(canonical_name, aliases.get(canonical_name))
        if existing is not None and existing != quant_type:
            raise ValueError(
                "Found conflicting DSpark quantization entries for "
                f"{name!r} and {canonical_name!r}: {quant_type!r} != {existing!r}."
            )
        aliases[canonical_name] = quant_type
...
class AscendModelSlimConfig(QuantizationConfig):
    ...
    def __init__(self, quant_config: dict[str, Any] | None = None):
        ...
        # Keep the on-disk ModelSlim manifest separate from the expanded
        # runtime lookup table.  _apply_extra_quant_adaptations() adds broad
        # compatibility aliases (hundreds of thousands for DSpark); those
        # aliases are not physical checkpoint tensor names.
        self.source_quant_description = copy.deepcopy(self.quant_description)
    ...
    def _map_deepseek_v4_dspark_mtp_prefix(
        self,
        prefix: str,
        packed_modules_mapping: Mapping[str, list[str]],
        num_hidden_layers: int | None = None,
    ) -> str | None:
        if "model.mtp.0.self_attn.wq_a.weight" not in self.quant_description:
            return None

        match = _DEEPSEEK_V4_DSPARK_MTP_LAYER_RE.match(prefix)
        if match is None:
            return None
        ...
        mtp_idx = layer_id - num_hidden_layers
        if mtp_idx < 0:
            return None

        candidate = f"model.mtp.{mtp_idx}.{match.group(2)}"
    ...
    def quant_prefix_mapper(
        ...
        if model_type in ("deepseek_v4", "deepseek_mtp"):
            ...
            if not self._has_quant_weight(prefix, packed_modules_mapping):
                candidate = self._map_deepseek_v4_dspark_mtp_prefix(
                    prefix,
    ...
    def _apply_extra_quant_adaptations(self) -> None:
        ...
        """
        _add_deepseek_v4_dspark_quant_aliases(self.quant_description)
