...
#       free-list/front-insertion behavior, add a `block_mask` argument to
#       `cache_full_blocks`, and inject the SWA retention three-state mask
#       (`reachable_block_mask` + `cache_blocks`). Each patch is guarded by
#       capability/source checks so it is skipped once the underlying vLLM
#       already provides the upstream behavior.
