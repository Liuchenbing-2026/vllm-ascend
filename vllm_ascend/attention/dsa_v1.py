...
class AscendDSAImpl(DSAAttentionImpl):
            ...
            if self.compress_ratio == 4:
                ...
                if self.skip_topk:
                    compress_topk_idxs = self._get_indexcache_topk_indices(
                        prefill_num_tokens, offset=prefill_offset
                    )
                else:
                    ...
                    if self.use_index_cache:
                        self._update_indexcache_topk_indices(
                            compress_topk_idxs, offset=prefill_offset
                        )
    ...
    def dsa_dual_stream_finish(
        ...
        if has_decode:
            ...
            if self.skip_topk:
                decode_topk_idxs = self._get_indexcache_topk_indices(
                    num_decode_tokens, offset=0
                )
            else:
        ...
        if has_prefill:
            ...
            if self.skip_topk:
                prefill_topk_idxs = self._get_indexcache_topk_indices(
                    prefill_num_tokens, offset=num_decode_tokens
                )
            else:
                ...
                if self.use_index_cache:
                    self._update_indexcache_topk_indices(
                        prefill_topk_idxs, offset=num_decode_tokens
                    )
