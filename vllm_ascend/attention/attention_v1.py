...
class AscendAttentionBackendImpl(AttentionImpl):
    ...
    def forward_fused_infer_attention(
        ...
        if self.head_size == FIA_TND_LARGE_HEAD_FALLBACK_HEAD_SIZE:
    ...
    def _forward_large_head_prefill_attention(
        ...
        """Run 512-dim Gemma4 prefill with FA because A2 FIA rejects it."""
        num_tokens = attn_metadata.actual_seq_lengths_q[-1]
        if (
            attn_metadata.attn_state == AscendAttentionState.PrefillNoCache
            ...
            dense_key = key[:num_tokens]
            dense_value = value[:num_tokens]
            actual_seq_lengths_kv = attn_metadata.actual_seq_lengths_q
        else:
        ...
        attn_output = torch_npu.npu_fusion_attention(
            ...
            atten_mask=attn_mask,
            scale=self.scale,
            pre_tockens=SWA_INT_MAX,
            next_tockens=0 if attn_metadata.causal else SWA_INT_MAX,
            actual_seq_qlen=attn_metadata.actual_seq_lengths_q,
            actual_seq_kvlen=actual_seq_lengths_kv,
            sparse_mode=sparse_mode,
        )[0]
    ...
    def _gather_paged_kv_to_dense(
        ...
        if seq_lens_device is None:
            raise RuntimeError("Gemma4 dense KV gather requires device sequence lengths")
        # Reuse the model runner's dynamically updated device tensor. Creating
        # torch.tensor(seq_lens, device=NPU) in this compiled replay path emits
        # a per-step Host-to-NPU CopyKernel that can fail with MTE out-of-range.
        seq_lens_tensor = seq_lens_device[: len(seq_lens)]
        valid_block_counts = (seq_lens_tensor + block_size - 1) // block_size
        block_offsets = torch.arange(
            num_blocks,
        ...
        valid_block_mask = block_offsets.unsqueeze(0) < valid_block_counts.unsqueeze(1)
        # Graph replay can leave stale values, including positive block IDs, in
        # block-table slots beyond a shorter row's real sequence length. Replace
        # all padded slots before index_select; the token mask below removes the
        # corresponding placeholder data from the dense result.
        flat_block_ids = block_table.masked_fill(~valid_block_mask, 0).reshape(-1).clamp_min_(0)
