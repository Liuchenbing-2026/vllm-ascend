...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def forward(
        ...
        # TP=8 (or any DP/TP > 1) num_input_tokens is the prefill batch token
        # count pre-shard while q_pe is already per-rank, so feeding the global
        # number causes "sparse_indices shape [N*tp, 1, 2048] expected
        # [N, 1, 2048]" tiling errors (TP=8 → N×8 vs N). PD混步 happens to be
        # safe because q_pe.shape[0] == num_input_tokens there, so this is a
        # no-op for it.
