...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['a_log', 'a', 'b', 'dt_bias']
    output_names = ['g', 'beta_output']
...
def _build_args(a_log_in__, a_in__, b_in__, dt_bias_in__, g_out_, beta_output_out_, beta, threshold):
    __inputs__ = []
    for arg in [a_log_in__, a_in__, b_in__, dt_bias_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [g_out_, beta_output_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("FusedGdnGating", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def fused_gdn_gating(a_log_in__, a_in__, b_in__, dt_bias_in__, g_out_, beta_output_out_, beta=1, threshold=20, kernel_name="fused_gdn_gating", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(a_log_in__, a_in__, b_in__, dt_bias_in__, g_out_, beta_output_out_, beta, threshold)
    ...
    origin_func_name = "fused_gdn_gating"
    ascendc_src_dir_ex = "fused_gdn_gating"
    ascendc_src_dir = "fused_gdn_gating"
    ascendc_src_file = "fused_gdn_gating.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator FusedGdnGating, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "FusedGdnGating"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[a_log_in__, a_in__, b_in__, dt_bias_in__], origin_outputs = [g_out_, beta_output_out_],\
...
def op_select_format(a_log_in__, a_in__, b_in__, dt_bias_in__, g_out_, beta_output_out_, beta=1, threshold=20, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_log_in__, a_in__, b_in__, dt_bias_in__, g_out_, beta_output_out_, beta, threshold)
    result = check_op_cap("op_select_format", "FusedGdnGating", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(a_log_in__, a_in__, b_in__, dt_bias_in__, g_out_, beta_output_out_, beta=1, threshold=20, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_log_in__, a_in__, b_in__, dt_bias_in__, g_out_, beta_output_out_, beta, threshold)
    result = check_op_cap("get_op_specific_info", "FusedGdnGating", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
