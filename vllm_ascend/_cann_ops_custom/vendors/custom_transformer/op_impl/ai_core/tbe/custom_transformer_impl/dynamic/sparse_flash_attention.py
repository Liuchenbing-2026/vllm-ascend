...
def get_dtype_fmt_options(__inputs__, __outputs__):
    ...
    output_names = ['attention_out', 'softmax_max', 'softmax_sum']
...
def _build_args(query_in__, key_in__, value_in__, sparse_indices_in__, block_table_in__, actual_seq_lengths_query_in__, actual_seq_lengths_kv_in__, query_rope_in__, key_rope_in__, attention_out_out_, softmax_max_out_, softmax_sum_out_, scale_value, sparse_block_size, layout_query, layout_kv, sparse_mode, pre_tokens, next_tokens, attention_mode, return_softmax_lse):
    __inputs__ = []
    for arg in [query_in__, key_in__, value_in__, sparse_indices_in__, block_table_in__, actual_seq_lengths_query_in__, actual_seq_lengths_kv_in__, query_rope_in__, key_rope_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [attention_out_out_, softmax_max_out_, softmax_sum_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if attention_mode != None:
        attr = {}
        attr["name"] = "attention_mode"
        attr["dtype"] = "int"
        attr["value"] = attention_mode
        __attrs__.append(attr)
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def sparse_flash_attention(query_in__, key_in__, value_in__, sparse_indices_in__, block_table_in__=None, actual_seq_lengths_query_in__=None, actual_seq_lengths_kv_in__=None, query_rope_in__=None, key_rope_in__=None, attention_out_out_=None, softmax_max_out_=None, softmax_sum_out_=None, scale_value=0.0, sparse_block_size=1, layout_query="BSND", layout_kv="BSND", sparse_mode=3, pre_tokens=9223372036854775807, next_tokens=9223372036854775807, attention_mode=2, return_softmax_lse=False, kernel_name="sparse_flash_attention", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, sparse_indices_in__, block_table_in__, actual_seq_lengths_query_in__, actual_seq_lengths_kv_in__, query_rope_in__, key_rope_in__, attention_out_out_, softmax_max_out_, softmax_sum_out_, scale_value, sparse_block_size, layout_query, layout_kv, sparse_mode, pre_tokens, next_tokens, attention_mode, return_softmax_lse)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[query_in__, key_in__, value_in__, sparse_indices_in__, block_table_in__, actual_seq_lengths_query_in__, actual_seq_lengths_kv_in__, query_rope_in__, key_rope_in__], origin_outputs = [attention_out_out_, softmax_max_out_, softmax_sum_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'optional', 'required', 'required', 'required'], init_value_list = [None, None, None],\
