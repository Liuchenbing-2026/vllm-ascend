...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['q', 'ori_kv', 'cmp_kv', 'ori_sparse_indices', 'cmp_sparse_indices', 'ori_block_table', 'cmp_block_table', 'cu_seqlens_q', 'cu_seqlens_ori_kv', 'cu_seqlens_cmp_kv', 'seqused_q', 'seqused_kv', 'sinks', 'metadata']
    output_names = ['attn_out', 'softmax_lse']
...
def _build_args(q_in__, ori_kv_in__, cmp_kv_in__, ori_sparse_indices_in__, cmp_sparse_indices_in__, ori_block_table_in__, cmp_block_table_in__, cu_seqlens_q_in__, cu_seqlens_ori_kv_in__, cu_seqlens_cmp_kv_in__, seqused_q_in__, seqused_kv_in__, sinks_in__, metadata_in__, attn_out_out_, softmax_lse_out_, softmax_scale, cmp_ratio, ori_mask_mode, cmp_mask_mode, ori_kv_stride, cmp_kv_stride, ori_win_left, ori_win_right, layout_q, layout_kv, return_softmax_lse):
    __inputs__ = []
    for arg in [q_in__, ori_kv_in__, cmp_kv_in__, ori_sparse_indices_in__, cmp_sparse_indices_in__, ori_block_table_in__, cmp_block_table_in__, cu_seqlens_q_in__, cu_seqlens_ori_kv_in__, cu_seqlens_cmp_kv_in__, seqused_q_in__, seqused_kv_in__, sinks_in__, metadata_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [attn_out_out_, softmax_lse_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if softmax_scale != None:
        attr = {}
        attr["name"] = "softmax_scale"
        attr["dtype"] = "float"
        attr["value"] = softmax_scale
        __attrs__.append(attr)
    if cmp_ratio != None:
        attr = {}
        attr["name"] = "cmp_ratio"
        attr["dtype"] = "int"
        attr["value"] = cmp_ratio
        __attrs__.append(attr)
    if ori_mask_mode != None:
        attr = {}
        attr["name"] = "ori_mask_mode"
        attr["dtype"] = "int"
        attr["value"] = ori_mask_mode
        __attrs__.append(attr)
    if cmp_mask_mode != None:
        attr = {}
        attr["name"] = "cmp_mask_mode"
        attr["dtype"] = "int"
        attr["value"] = cmp_mask_mode
        __attrs__.append(attr)
    if ori_kv_stride != None:
        attr = {}
        attr["name"] = "ori_kv_stride"
        attr["dtype"] = "int"
        attr["value"] = ori_kv_stride
        __attrs__.append(attr)
    if cmp_kv_stride != None:
        attr = {}
        attr["name"] = "cmp_kv_stride"
        attr["dtype"] = "int"
        attr["value"] = cmp_kv_stride
        __attrs__.append(attr)
    if ori_win_left != None:
        attr = {}
        attr["name"] = "ori_win_left"
        attr["dtype"] = "int"
        attr["value"] = ori_win_left
        __attrs__.append(attr)
    if ori_win_right != None:
        attr = {}
        attr["name"] = "ori_win_right"
        attr["dtype"] = "int"
        attr["value"] = ori_win_right
        __attrs__.append(attr)
    if layout_q != None:
        attr = {}
        attr["name"] = "layout_q"
    ...
    if layout_kv != None:
        ...
        __attrs__.append(attr)
    if return_softmax_lse != None:
        attr = {}
        attr["name"] = "return_softmax_lse"
        attr["dtype"] = "bool"
        attr["value"] = return_softmax_lse
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("SparseAttnSharedkv", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def sparse_attn_sharedkv(q_in__, ori_kv_in__=None, cmp_kv_in__=None, ori_sparse_indices_in__=None, cmp_sparse_indices_in__=None, ori_block_table_in__=None, cmp_block_table_in__=None, cu_seqlens_q_in__=None, cu_seqlens_ori_kv_in__=None, cu_seqlens_cmp_kv_in__=None, seqused_q_in__=None, seqused_kv_in__=None, sinks_in__=None, metadata_in__=None, attn_out_out_=None, softmax_lse_out_=None, softmax_scale=0.0, cmp_ratio=0, ori_mask_mode=0, cmp_mask_mode=0, ori_kv_stride=0, cmp_kv_stride=0, ori_win_left=127, ori_win_right=0, layout_q="BSND", layout_kv="PA_ND", return_softmax_lse=False, kernel_name="sparse_attn_sharedkv", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, ori_kv_in__, cmp_kv_in__, ori_sparse_indices_in__, cmp_sparse_indices_in__, ori_block_table_in__, cmp_block_table_in__, cu_seqlens_q_in__, cu_seqlens_ori_kv_in__, cu_seqlens_cmp_kv_in__, seqused_q_in__, seqused_kv_in__, sinks_in__, metadata_in__, attn_out_out_, softmax_lse_out_, softmax_scale, cmp_ratio, ori_mask_mode, cmp_mask_mode, ori_kv_stride, cmp_kv_stride, ori_win_left, ori_win_right, layout_q, layout_kv, return_softmax_lse)
    ...
    custom_all_compile_options = {'__ALLSOC__': ['-Wno-ignored-attributes', '-include/tmp/vllm-ascend/csrc/common/include/cann_compat.h']},
    ...
    origin_func_name = "sparse_attn_sharedkv"
    ascendc_src_dir_ex = "sparse_attn_sharedkv"
    ascendc_src_dir = "sparse_attn_sharedkv"
    ascendc_src_file = "sparse_attn_sharedkv.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator SparseAttnSharedkv, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SparseAttnSharedkv"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[q_in__, ori_kv_in__, cmp_kv_in__, ori_sparse_indices_in__, cmp_sparse_indices_in__, ori_block_table_in__, cmp_block_table_in__, cu_seqlens_q_in__, cu_seqlens_ori_kv_in__, cu_seqlens_cmp_kv_in__, seqused_q_in__, seqused_kv_in__, sinks_in__, metadata_in__], origin_outputs = [attn_out_out_, softmax_lse_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(q_in__, ori_kv_in__=None, cmp_kv_in__=None, ori_sparse_indices_in__=None, cmp_sparse_indices_in__=None, ori_block_table_in__=None, cmp_block_table_in__=None, cu_seqlens_q_in__=None, cu_seqlens_ori_kv_in__=None, cu_seqlens_cmp_kv_in__=None, seqused_q_in__=None, seqused_kv_in__=None, sinks_in__=None, metadata_in__=None, attn_out_out_=None, softmax_lse_out_=None, softmax_scale=0.0, cmp_ratio=0, ori_mask_mode=0, cmp_mask_mode=0, ori_kv_stride=0, cmp_kv_stride=0, ori_win_left=127, ori_win_right=0, layout_q="BSND", layout_kv="PA_ND", return_softmax_lse=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, ori_kv_in__, cmp_kv_in__, ori_sparse_indices_in__, cmp_sparse_indices_in__, ori_block_table_in__, cmp_block_table_in__, cu_seqlens_q_in__, cu_seqlens_ori_kv_in__, cu_seqlens_cmp_kv_in__, seqused_q_in__, seqused_kv_in__, sinks_in__, metadata_in__, attn_out_out_, softmax_lse_out_, softmax_scale, cmp_ratio, ori_mask_mode, cmp_mask_mode, ori_kv_stride, cmp_kv_stride, ori_win_left, ori_win_right, layout_q, layout_kv, return_softmax_lse)
    result = check_op_cap("op_select_format", "SparseAttnSharedkv", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(q_in__, ori_kv_in__=None, cmp_kv_in__=None, ori_sparse_indices_in__=None, cmp_sparse_indices_in__=None, ori_block_table_in__=None, cmp_block_table_in__=None, cu_seqlens_q_in__=None, cu_seqlens_ori_kv_in__=None, cu_seqlens_cmp_kv_in__=None, seqused_q_in__=None, seqused_kv_in__=None, sinks_in__=None, metadata_in__=None, attn_out_out_=None, softmax_lse_out_=None, softmax_scale=0.0, cmp_ratio=0, ori_mask_mode=0, cmp_mask_mode=0, ori_kv_stride=0, cmp_kv_stride=0, ori_win_left=127, ori_win_right=0, layout_q="BSND", layout_kv="PA_ND", return_softmax_lse=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, ori_kv_in__, cmp_kv_in__, ori_sparse_indices_in__, cmp_sparse_indices_in__, ori_block_table_in__, cmp_block_table_in__, cu_seqlens_q_in__, cu_seqlens_ori_kv_in__, cu_seqlens_cmp_kv_in__, seqused_q_in__, seqused_kv_in__, sinks_in__, metadata_in__, attn_out_out_, softmax_lse_out_, softmax_scale, cmp_ratio, ori_mask_mode, cmp_mask_mode, ori_kv_stride, cmp_kv_stride, ori_win_left, ori_win_right, layout_q, layout_kv, return_softmax_lse)
    result = check_op_cap("get_op_specific_info", "SparseAttnSharedkv", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
