...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['q', 'k', 'v', 'h', 'g', 'cu_seqlens', 'chunk_offsets']
    output_names = ['o']
...
def _build_args(q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__, chunk_offsets_in__, o_out_, scale, chunk_size):
    __inputs__ = []
    for arg in [q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__, chunk_offsets_in__]:
        if arg != None:
...
@tbe_register.register_operator("ChunkFwdO", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def chunk_fwd_o(q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__=None, chunk_offsets_in__=None, o_out_=None, scale=0.0, chunk_size=0, kernel_name="chunk_fwd_o", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__, chunk_offsets_in__, o_out_, scale, chunk_size)
    ...
    origin_func_name = "chunk_fwd_o"
    ascendc_src_dir_ex = "chunk_fwd_o"
    ascendc_src_dir = "chunk_fwd_o"
    ascendc_src_file = "chunk_fwd_o.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator ChunkFwdO, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ChunkFwdO"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__, chunk_offsets_in__], origin_outputs = [o_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'required'], init_value_list = [None],\
...
def op_select_format(q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__=None, chunk_offsets_in__=None, o_out_=None, scale=0.0, chunk_size=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__, chunk_offsets_in__, o_out_, scale, chunk_size)
    result = check_op_cap("op_select_format", "ChunkFwdO", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__=None, chunk_offsets_in__=None, o_out_=None, scale=0.0, chunk_size=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, k_in__, v_in__, h_in__, g_in__, cu_seqlens_in__, chunk_offsets_in__, o_out_, scale, chunk_size)
    result = check_op_cap("get_op_specific_info", "ChunkFwdO", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
