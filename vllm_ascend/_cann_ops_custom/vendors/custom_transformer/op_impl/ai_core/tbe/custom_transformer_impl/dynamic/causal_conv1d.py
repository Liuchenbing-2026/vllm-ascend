...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight', 'bias', 'convStates', 'queryStartLoc', 'cacheIndices', 'initialStateMode', 'numAcceptedTokens']
    output_names = ['y']
...
def _build_args(x_in__, weight_in__, bias_in__, convStates_in__, queryStartLoc_in__, cacheIndices_in__, initialStateMode_in__, numAcceptedTokens_in__, y_out_, activationMode, padSlotId, runMode):
    __inputs__ = []
    for arg in [x_in__, weight_in__, bias_in__, convStates_in__, queryStartLoc_in__, cacheIndices_in__, initialStateMode_in__, numAcceptedTokens_in__]:
        if arg != None:
    ...
    if activationMode != None:
        attr = {}
        attr["name"] = "activationMode"
        attr["dtype"] = "int"
        attr["value"] = activationMode
        __attrs__.append(attr)
    if padSlotId != None:
        attr = {}
        attr["name"] = "padSlotId"
        attr["dtype"] = "int"
        attr["value"] = padSlotId
        __attrs__.append(attr)
    if runMode != None:
        attr = {}
        attr["name"] = "runMode"
...
@tbe_register.register_operator("CausalConv1d", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def causal_conv1d(x_in__, weight_in__, bias_in__=None, convStates_in__=None, queryStartLoc_in__=None, cacheIndices_in__=None, initialStateMode_in__=None, numAcceptedTokens_in__=None, y_out_=None, activationMode=0, padSlotId=-1, runMode=0, kernel_name="causal_conv1d", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, convStates_in__, queryStartLoc_in__, cacheIndices_in__, initialStateMode_in__, numAcceptedTokens_in__, y_out_, activationMode, padSlotId, runMode)
    ...
    origin_func_name = "causal_conv1d"
    ascendc_src_dir_ex = "causal_conv1d"
    ascendc_src_dir = "causal_conv1d"
    ascendc_src_file = "causal_conv1d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator CausalConv1d, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, bias_in__, convStates_in__, queryStartLoc_in__, cacheIndices_in__, initialStateMode_in__, numAcceptedTokens_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'required', 'optional', 'optional', 'optional', 'optional', 'required'], init_value_list = [None],\
