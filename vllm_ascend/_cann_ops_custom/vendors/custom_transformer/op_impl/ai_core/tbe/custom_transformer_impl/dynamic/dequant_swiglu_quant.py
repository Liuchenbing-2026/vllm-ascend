...
def _build_args(x_in__, weight_scale_in__, activation_scale_in__, bias_in__, quant_scale_in__, quant_offset_in__, group_index_in__, y_out_, scale_out_, activate_left, quant_mode, dst_type, round_mode, activate_dim, swiglu_mode, clamp_limit, glu_alpha, glu_bias):
    __inputs__ = []
    ...
    if round_mode != None:
        ...
        __attrs__.append(attr)
    if activate_dim != None:
        attr = {}
        attr["name"] = "activate_dim"
        attr["dtype"] = "int"
        attr["value"] = activate_dim
        __attrs__.append(attr)
    if swiglu_mode != None:
        attr = {}
        attr["name"] = "swiglu_mode"
        attr["dtype"] = "int"
        attr["value"] = swiglu_mode
        __attrs__.append(attr)
    if clamp_limit != None:
        attr = {}
        attr["name"] = "clamp_limit"
        attr["dtype"] = "float"
        attr["value"] = clamp_limit
        __attrs__.append(attr)
    if glu_alpha != None:
        attr = {}
        attr["name"] = "glu_alpha"
        attr["dtype"] = "float"
        attr["value"] = glu_alpha
        __attrs__.append(attr)
    if glu_bias != None:
        attr = {}
        attr["name"] = "glu_bias"
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def dequant_swiglu_quant(x_in__, weight_scale_in__=None, activation_scale_in__=None, bias_in__=None, quant_scale_in__=None, quant_offset_in__=None, group_index_in__=None, y_out_=None, scale_out_=None, activate_left=False, quant_mode="static", dst_type=2, round_mode="rint", activate_dim=-1, swiglu_mode=0, clamp_limit=0, glu_alpha=1, glu_bias=0, kernel_name="dequant_swiglu_quant", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_scale_in__, activation_scale_in__, bias_in__, quant_scale_in__, quant_offset_in__, group_index_in__, y_out_, scale_out_, activate_left, quant_mode, dst_type, round_mode, activate_dim, swiglu_mode, clamp_limit, glu_alpha, glu_bias)
    ...
    msg = "start compile Ascend C Operator DequantSwigluQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
