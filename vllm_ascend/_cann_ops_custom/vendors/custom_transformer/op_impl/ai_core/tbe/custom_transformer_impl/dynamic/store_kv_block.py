...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['keyIn', 'keyCacheIn', 'groupLen', 'groupKeyIdx', 'groupKeyCacheIdx']
    output_names = []
...
def _build_args(keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__, blockSize):
    __inputs__ = []
    for arg in [keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__]:
        if arg != None:
...
@tbe_register.register_operator("StoreKVBlock", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def store_kv_block(keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__, blockSize, kernel_name="store_kv_block", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__, blockSize)
    ...
    origin_func_name = "store_kv_block"
    ascendc_src_dir_ex = "store_kv_block"
    ascendc_src_dir = "store_k_v_block"
    ascendc_src_file = "store_kv_block.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator StoreKVBlock, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "StoreKVBlock"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__], origin_outputs = [],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required'], init_value_list = [],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__, blockSize, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__, blockSize)
    result = check_op_cap("op_select_format", "StoreKVBlock", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__, blockSize, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(keyIn_in__, keyCacheIn_in__, groupLen_in__, groupKeyIdx_in__, groupKeyCacheIdx_in__, blockSize)
    result = check_op_cap("get_op_specific_info", "StoreKVBlock", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
