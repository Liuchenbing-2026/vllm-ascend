...
def _build_args(x_in__, cos_in__, sin_in__, x_out_, mode, partial_slice):
    __inputs__ = []
    ...
    if partial_slice != None:
        attr = {}
        attr["name"] = "partial_slice"
        attr["dtype"] = "list_int"
        attr["value"] = partial_slice
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("InplacePartialRotaryMul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_LIST_INT, para_check.KERNEL_NAME)
def inplace_partial_rotary_mul(x_in__, cos_in__, sin_in__, x_out_, mode=0, partial_slice=[0,0], kernel_name="inplace_partial_rotary_mul", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, x_out_, mode, partial_slice)
    ...
    origin_func_name = "inplace_partial_rotary_mul"
    ascendc_src_dir_ex = "inplace_partial_rotary_mul"
    ascendc_src_dir = "inplace_partial_rotary_mul"
    ascendc_src_file = "inplace_partial_rotary_mul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator InplacePartialRotaryMul, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, cos_in__, sin_in__], origin_outputs = [x_out_],\
