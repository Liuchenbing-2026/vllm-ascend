...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'x_scale', 'group_list', 'weight', 'weight_scale', 'weight_assist_matrix', 'bias', 'smooth_scale']
...
def _build_args(x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__, smooth_scale_in__, y_out_, y_scale_out_, dequant_mode, dequant_dtype, quant_mode, quant_dtype, transpose_weight, group_list_type, tuning_config, swiglu_limit):
    __inputs__ = []
    for arg in [x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__, smooth_scale_in__]:
        if arg != None:
    ...
    for arg in [y_out_, y_scale_out_]:
        if arg != None:
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if dequant_mode != None:
        attr = {}
        attr["name"] = "dequant_mode"
        attr["dtype"] = "int"
        attr["value"] = dequant_mode
        __attrs__.append(attr)
    if dequant_dtype != None:
        attr = {}
        attr["name"] = "dequant_dtype"
        attr["dtype"] = "int"
        attr["value"] = dequant_dtype
        __attrs__.append(attr)
    if quant_mode != None:
        attr = {}
    ...
    if quant_dtype != None:
        attr = {}
        attr["name"] = "quant_dtype"
        attr["dtype"] = "int"
        attr["value"] = quant_dtype
        __attrs__.append(attr)
    ...
    if group_list_type != None:
        ...
        __attrs__.append(attr)
    if tuning_config != None:
        attr = {}
        attr["name"] = "tuning_config"
        attr["dtype"] = "list_int"
        attr["value"] = tuning_config
        __attrs__.append(attr)
...
@tbe_register.register_operator("GroupedMatmulSwigluQuantV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def grouped_matmul_swiglu_quant_v2(x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__=None, smooth_scale_in__=None, y_out_=None, y_scale_out_=None, dequant_mode=0, dequant_dtype=0, quant_mode=0, quant_dtype=0, transpose_weight=False, group_list_type=0, tuning_config=[0], swiglu_limit=0, kernel_name="grouped_matmul_swiglu_quant_v2", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__, smooth_scale_in__, y_out_, y_scale_out_, dequant_mode, dequant_dtype, quant_mode, quant_dtype, transpose_weight, group_list_type, tuning_config, swiglu_limit)
    ...
    origin_func_name = "grouped_matmul_swiglu_quant_v2"
    ascendc_src_dir_ex = "grouped_matmul_swiglu_quant_v2"
    ascendc_src_dir = "grouped_matmul_swiglu_quant_v2"
    ascendc_src_file = "grouped_matmul_swiglu_quant_v2_apt.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator GroupedMatmulSwigluQuantV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__, smooth_scale_in__], origin_outputs = [y_out_, y_scale_out_],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'dynamic', 'dynamic', 'dynamic', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__=None, smooth_scale_in__=None, y_out_=None, y_scale_out_=None, dequant_mode=0, dequant_dtype=0, quant_mode=0, quant_dtype=0, transpose_weight=False, group_list_type=0, tuning_config=[0], swiglu_limit=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__, smooth_scale_in__, y_out_, y_scale_out_, dequant_mode, dequant_dtype, quant_mode, quant_dtype, transpose_weight, group_list_type, tuning_config, swiglu_limit)
    result = check_op_cap("op_select_format", "GroupedMatmulSwigluQuantV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__=None, smooth_scale_in__=None, y_out_=None, y_scale_out_=None, dequant_mode=0, dequant_dtype=0, quant_mode=0, quant_dtype=0, transpose_weight=False, group_list_type=0, tuning_config=[0], swiglu_limit=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, x_scale_in__, group_list_in__, weight_in__, weight_scale_in__, weight_assist_matrix_in__, bias_in__, smooth_scale_in__, y_out_, y_scale_out_, dequant_mode, dequant_dtype, quant_mode, quant_dtype, transpose_weight, group_list_type, tuning_config, swiglu_limit)
    result = check_op_cap("get_op_specific_info", "GroupedMatmulSwigluQuantV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
