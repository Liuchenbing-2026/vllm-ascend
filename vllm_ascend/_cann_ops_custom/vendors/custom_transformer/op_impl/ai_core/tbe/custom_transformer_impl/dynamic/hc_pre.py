...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'hc_fn', 'hc_scale', 'hc_base']
    output_names = ['y', 'post', 'comb_frag']
...
def _build_args(x_in__, hc_fn_in__, hc_scale_in__, hc_base_in__, y_out_, post_out_, comb_frag_out_, hc_mult, hc_sinkhorn_iters, hc_eps, norm_eps):
    __inputs__ = []
    for arg in [x_in__, hc_fn_in__, hc_scale_in__, hc_base_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, post_out_, comb_frag_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if hc_sinkhorn_iters != None:
        attr = {}
        attr["name"] = "hc_sinkhorn_iters"
        attr["dtype"] = "int"
        attr["value"] = hc_sinkhorn_iters
        __attrs__.append(attr)
    if hc_eps != None:
        attr = {}
        attr["name"] = "hc_eps"
...
@tbe_register.register_operator("HcPre", trans_bool_to_s8=False)
...
def hc_pre(x_in__, hc_fn_in__, hc_scale_in__, hc_base_in__, y_out_, post_out_, comb_frag_out_, hc_mult=4, hc_sinkhorn_iters=20, hc_eps=1e-06, norm_eps=1e-06, kernel_name="hc_pre", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, hc_fn_in__, hc_scale_in__, hc_base_in__, y_out_, post_out_, comb_frag_out_, hc_mult, hc_sinkhorn_iters, hc_eps, norm_eps)
    ...
    origin_func_name = "hc_pre"
    ascendc_src_dir_ex = "hc_pre"
    ascendc_src_dir = "hc_pre"
    ascendc_src_file = "hc_pre.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator HcPre, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "HcPre"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, hc_fn_in__, hc_scale_in__, hc_base_in__], origin_outputs = [y_out_, post_out_, comb_frag_out_],\
