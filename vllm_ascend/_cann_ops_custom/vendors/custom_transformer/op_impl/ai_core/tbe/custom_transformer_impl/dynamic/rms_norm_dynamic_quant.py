...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'gamma', 'smooth_scale1', 'smooth_scale2', 'beta']
    output_names = ['y1', 'y2', 'scale1', 'scale2']
...
def _build_args(x_in__, gamma_in__, smooth_scale1_in__, smooth_scale2_in__, beta_in__, y1_out_, y2_out_, scale1_out_, scale2_out_, epsilon, output_mask, dst_type):
    __inputs__ = []
    for arg in [x_in__, gamma_in__, smooth_scale1_in__, smooth_scale2_in__, beta_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y1_out_, y2_out_, scale1_out_, scale2_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if output_mask != None:
        attr = {}
        attr["name"] = "output_mask"
        attr["dtype"] = "list_bool"
        attr["value"] = output_mask
        __attrs__.append(attr)
    if dst_type != None:
        attr = {}
        attr["name"] = "dst_type"
...
@tbe_register.register_operator("RmsNormDynamicQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_LIST_BOOL, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def rms_norm_dynamic_quant(x_in__, gamma_in__, smooth_scale1_in__=None, smooth_scale2_in__=None, beta_in__=None, y1_out_=None, y2_out_=None, scale1_out_=None, scale2_out_=None, epsilon=1e-06, output_mask=[], dst_type=2, kernel_name="rms_norm_dynamic_quant", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, smooth_scale1_in__, smooth_scale2_in__, beta_in__, y1_out_, y2_out_, scale1_out_, scale2_out_, epsilon, output_mask, dst_type)
    ...
    origin_func_name = "rms_norm_dynamic_quant"
    ascendc_src_dir_ex = "rms_norm_dynamic_quant"
    ascendc_src_dir = "rms_norm_dynamic_quant"
    ascendc_src_file = "rms_norm_dynamic_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator RmsNormDynamicQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, gamma_in__, smooth_scale1_in__, smooth_scale2_in__, beta_in__], origin_outputs = [y1_out_, y2_out_, scale1_out_, scale2_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'optional', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x_in__, gamma_in__, smooth_scale1_in__=None, smooth_scale2_in__=None, beta_in__=None, y1_out_=None, y2_out_=None, scale1_out_=None, scale2_out_=None, epsilon=1e-06, output_mask=[], dst_type=2, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, smooth_scale1_in__, smooth_scale2_in__, beta_in__, y1_out_, y2_out_, scale1_out_, scale2_out_, epsilon, output_mask, dst_type)
    result = check_op_cap("op_select_format", "RmsNormDynamicQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gamma_in__, smooth_scale1_in__=None, smooth_scale2_in__=None, beta_in__=None, y1_out_=None, y2_out_=None, scale1_out_=None, scale2_out_=None, epsilon=1e-06, output_mask=[], dst_type=2, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, smooth_scale1_in__, smooth_scale2_in__, beta_in__, y1_out_, y2_out_, scale1_out_, scale2_out_, epsilon, output_mask, dst_type)
    result = check_op_cap("get_op_specific_info", "RmsNormDynamicQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
