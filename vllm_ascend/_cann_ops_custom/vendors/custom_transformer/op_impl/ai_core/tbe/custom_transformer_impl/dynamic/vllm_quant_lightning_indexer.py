...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['query', 'key', 'weights', 'query_dequant_scale', 'key_dequant_scale', 'actual_seq_lengths_query', 'actual_seq_lengths_key', 'block_table', 'metadata']
    output_names = ['sparse_indices', 'sparse_values']
...
def _build_args(query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__, actual_seq_lengths_key_in__, block_table_in__, metadata_in__, sparse_indices_out_, sparse_values_out_, query_quant_mode, key_quant_mode, layout_query, layout_key, sparse_count, sparse_mode, pre_tokens, next_tokens, cmp_ratio, return_values, stride, scale_stride):
    __inputs__ = []
    for arg in [query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__, actual_seq_lengths_key_in__, block_table_in__, metadata_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [sparse_indices_out_, sparse_values_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if key_quant_mode != None:
        attr = {}
        attr["name"] = "key_quant_mode"
        attr["dtype"] = "int"
        attr["value"] = key_quant_mode
        __attrs__.append(attr)
    ...
    if return_values != None:
        attr = {}
        attr["name"] = "return_values"
        attr["dtype"] = "bool"
        attr["value"] = return_values
        __attrs__.append(attr)
    if stride != None:
        attr = {}
        attr["name"] = "stride"
        ...
        attr["value"] = stride
        __attrs__.append(attr)
    if scale_stride != None:
        attr = {}
        attr["name"] = "scale_stride"
        attr["dtype"] = "int"
        attr["value"] = scale_stride
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("VllmQuantLightningIndexer", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def vllm_quant_lightning_indexer(query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__=None, actual_seq_lengths_key_in__=None, block_table_in__=None, metadata_in__=None, sparse_indices_out_=None, sparse_values_out_=None, query_quant_mode=0, key_quant_mode=0, layout_query="BSND", layout_key="BSND", sparse_count=2048, sparse_mode=3, pre_tokens=9223372036854775807, next_tokens=9223372036854775807, cmp_ratio=1, return_values=False, stride=1, scale_stride=1, kernel_name="vllm_quant_lightning_indexer", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__, actual_seq_lengths_key_in__, block_table_in__, metadata_in__, sparse_indices_out_, sparse_values_out_, query_quant_mode, key_quant_mode, layout_query, layout_key, sparse_count, sparse_mode, pre_tokens, next_tokens, cmp_ratio, return_values, stride, scale_stride)
    ...
    origin_func_name = "vllm_quant_lightning_indexer"
    ascendc_src_dir_ex = "vllm_quant_lightning_indexer"
    ascendc_src_dir = "vllm_quant_lightning_indexer"
    ascendc_src_file = "vllm_quant_lightning_indexer.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator VllmQuantLightningIndexer, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "VllmQuantLightningIndexer"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__, actual_seq_lengths_key_in__, block_table_in__, metadata_in__], origin_outputs = [sparse_indices_out_, sparse_values_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__=None, actual_seq_lengths_key_in__=None, block_table_in__=None, metadata_in__=None, sparse_indices_out_=None, sparse_values_out_=None, query_quant_mode=0, key_quant_mode=0, layout_query="BSND", layout_key="BSND", sparse_count=2048, sparse_mode=3, pre_tokens=9223372036854775807, next_tokens=9223372036854775807, cmp_ratio=1, return_values=False, stride=1, scale_stride=1, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__, actual_seq_lengths_key_in__, block_table_in__, metadata_in__, sparse_indices_out_, sparse_values_out_, query_quant_mode, key_quant_mode, layout_query, layout_key, sparse_count, sparse_mode, pre_tokens, next_tokens, cmp_ratio, return_values, stride, scale_stride)
    result = check_op_cap("op_select_format", "VllmQuantLightningIndexer", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__=None, actual_seq_lengths_key_in__=None, block_table_in__=None, metadata_in__=None, sparse_indices_out_=None, sparse_values_out_=None, query_quant_mode=0, key_quant_mode=0, layout_query="BSND", layout_key="BSND", sparse_count=2048, sparse_mode=3, pre_tokens=9223372036854775807, next_tokens=9223372036854775807, cmp_ratio=1, return_values=False, stride=1, scale_stride=1, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, weights_in__, query_dequant_scale_in__, key_dequant_scale_in__, actual_seq_lengths_query_in__, actual_seq_lengths_key_in__, block_table_in__, metadata_in__, sparse_indices_out_, sparse_values_out_, query_quant_mode, key_quant_mode, layout_query, layout_key, sparse_count, sparse_mode, pre_tokens, next_tokens, cmp_ratio, return_values, stride, scale_stride)
    result = check_op_cap("get_op_specific_info", "VllmQuantLightningIndexer", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
