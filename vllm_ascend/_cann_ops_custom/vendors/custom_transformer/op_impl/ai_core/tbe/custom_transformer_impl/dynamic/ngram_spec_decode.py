...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['tokenIds', 'numTokensNoSpec', 'sampledTokenIds', 'discardRequestMask']
    output_names = ['nextTokenIds', 'draftTokenIds', 'numValidDraftTokens']
...
def _build_args(tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__, nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_, vocab_size, min_n, max_n, k):
    __inputs__ = []
    for arg in [tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("NgramSpecDecode", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def ngram_spec_decode(tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__, nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_, vocab_size, min_n, max_n, k, kernel_name="ngram_spec_decode", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__, nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_, vocab_size, min_n, max_n, k)
    ...
    origin_func_name = "ngram_spec_decode"
    ascendc_src_dir_ex = "ngram_spec_decode"
    ascendc_src_dir = "ngram_spec_decode"
    ascendc_src_file = "ngram_spec_decode.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator NgramSpecDecode, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "NgramSpecDecode"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__], origin_outputs = [nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_],\
...
def op_select_format(tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__, nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_, vocab_size, min_n, max_n, k, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__, nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_, vocab_size, min_n, max_n, k)
    result = check_op_cap("op_select_format", "NgramSpecDecode", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__, nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_, vocab_size, min_n, max_n, k, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(tokenIds_in__, numTokensNoSpec_in__, sampledTokenIds_in__, discardRequestMask_in__, nextTokenIds_out_, draftTokenIds_out_, numValidDraftTokens_out_, vocab_size, min_n, max_n, k)
    result = check_op_cap("get_op_specific_info", "NgramSpecDecode", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
