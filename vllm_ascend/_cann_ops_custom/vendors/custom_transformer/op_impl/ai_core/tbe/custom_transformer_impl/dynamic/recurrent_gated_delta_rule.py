...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['query', 'key', 'value', 'beta', 'state', 'actual_seq_lengths', 'ssm_state_indices', 'g', 'gk', 'num_accepted_tokens']
    output_names = ['out', 'state']
...
def _build_args(query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__, gk_in__, num_accepted_tokens_in__, out_out_, state_out_, scale_value):
    __inputs__ = []
    for arg in [query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__, gk_in__, num_accepted_tokens_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [out_out_, state_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("RecurrentGatedDeltaRule", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def recurrent_gated_delta_rule(query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__=None, gk_in__=None, num_accepted_tokens_in__=None, out_out_=None, state_out_=None, scale_value=1, kernel_name="recurrent_gated_delta_rule", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__, gk_in__, num_accepted_tokens_in__, out_out_, state_out_, scale_value)
    ...
    origin_func_name = "recurrent_gated_delta_rule"
    ascendc_src_dir_ex = "recurrent_gated_delta_rule"
    ascendc_src_dir = "recurrent_gated_delta_rule"
    ascendc_src_file = "recurrent_gated_delta_rule.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator RecurrentGatedDeltaRule, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "RecurrentGatedDeltaRule"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__, gk_in__, num_accepted_tokens_in__], origin_outputs = [out_out_, state_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__=None, gk_in__=None, num_accepted_tokens_in__=None, out_out_=None, state_out_=None, scale_value=1, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__, gk_in__, num_accepted_tokens_in__, out_out_, state_out_, scale_value)
    result = check_op_cap("op_select_format", "RecurrentGatedDeltaRule", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__=None, gk_in__=None, num_accepted_tokens_in__=None, out_out_=None, state_out_=None, scale_value=1, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, beta_in__, state_in__, actual_seq_lengths_in__, ssm_state_indices_in__, g_in__, gk_in__, num_accepted_tokens_in__, out_out_, state_out_, scale_value)
    result = check_op_cap("get_op_specific_info", "RecurrentGatedDeltaRule", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
