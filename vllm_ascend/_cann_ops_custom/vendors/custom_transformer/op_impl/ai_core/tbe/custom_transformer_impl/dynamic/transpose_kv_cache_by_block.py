...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['KCache', 'VCache', 'blockIDs']
    output_names = []
...
def _build_args(KCache_in__, VCache_in__, blockIDs_in__, blockSize, headNum, headDim, splitNum, layerNum):
    __inputs__ = []
    for arg in [KCache_in__, VCache_in__, blockIDs_in__]:
        if arg != None:
...
@tbe_register.register_operator("TransposeKvCacheByBlock", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def transpose_kv_cache_by_block(KCache_in__, VCache_in__, blockIDs_in__, blockSize, headNum, headDim, splitNum, layerNum, kernel_name="transpose_kv_cache_by_block", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(KCache_in__, VCache_in__, blockIDs_in__, blockSize, headNum, headDim, splitNum, layerNum)
    ...
    origin_func_name = "transpose_kv_cache_by_block"
    ascendc_src_dir_ex = "transpose_kv_cache_by_block"
    ascendc_src_dir = "transpose_kv_cache_by_block"
    ascendc_src_file = "transpose_kv_cache_by_block.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator TransposeKvCacheByBlock, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "TransposeKvCacheByBlock"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[KCache_in__, VCache_in__, blockIDs_in__], origin_outputs = [],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['dynamic', 'dynamic', 'required'], init_value_list = [],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(KCache_in__, VCache_in__, blockIDs_in__, blockSize, headNum, headDim, splitNum, layerNum, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(KCache_in__, VCache_in__, blockIDs_in__, blockSize, headNum, headDim, splitNum, layerNum)
    result = check_op_cap("op_select_format", "TransposeKvCacheByBlock", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(KCache_in__, VCache_in__, blockIDs_in__, blockSize, headNum, headDim, splitNum, layerNum, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(KCache_in__, VCache_in__, blockIDs_in__, blockSize, headNum, headDim, splitNum, layerNum)
    result = check_op_cap("get_op_specific_info", "TransposeKvCacheByBlock", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
