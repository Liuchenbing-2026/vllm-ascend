...
def _build_args(x_in__, y_out_, epsilon):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("HcPreInvRms", trans_bool_to_s8=False)
...
def hc_pre_inv_rms(x_in__, y_out_, epsilon=1e-06, kernel_name="hc_pre_inv_rms", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, epsilon)
    ...
    origin_func_name = "hc_pre_inv_rms"
    ascendc_src_dir_ex = "hc_pre_inv_rms"
    ascendc_src_dir = "hc_pre_inv_rms"
    ascendc_src_file = "hc_pre_inv_rms.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator HcPreInvRms, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "HcPreInvRms"
