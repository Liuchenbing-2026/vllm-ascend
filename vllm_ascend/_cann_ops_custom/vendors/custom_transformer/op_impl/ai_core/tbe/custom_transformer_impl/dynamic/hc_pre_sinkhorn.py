...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['mixes', 'rsqrt', 'hc_scale', 'hc_base', 'x']
...
def _build_args(mixes_in__, rsqrt_in__, hc_scale_in__, hc_base_in__, x_in__, y_out_, post_out_, comb_frag_out_, hc_mult, hc_sinkhorn_iters, hc_eps):
    __inputs__ = []
    for arg in [mixes_in__, rsqrt_in__, hc_scale_in__, hc_base_in__, x_in__]:
        if arg != None:
...
@tbe_register.register_operator("HcPreSinkhorn", trans_bool_to_s8=False)
...
def hc_pre_sinkhorn(mixes_in__, rsqrt_in__, hc_scale_in__, hc_base_in__, x_in__, y_out_, post_out_, comb_frag_out_, hc_mult=4, hc_sinkhorn_iters=20, hc_eps=1e-06, kernel_name="hc_pre_sinkhorn", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(mixes_in__, rsqrt_in__, hc_scale_in__, hc_base_in__, x_in__, y_out_, post_out_, comb_frag_out_, hc_mult, hc_sinkhorn_iters, hc_eps)
    ...
    origin_func_name = "hc_pre_sinkhorn"
    ascendc_src_dir_ex = "hc_pre_sinkhorn"
    ascendc_src_dir = "hc_pre_sinkhorn"
    ascendc_src_file = "hc_pre_sinkhorn.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator HcPreSinkhorn, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "HcPreSinkhorn"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[mixes_in__, rsqrt_in__, hc_scale_in__, hc_base_in__, x_in__], origin_outputs = [y_out_, post_out_, comb_frag_out_],\
