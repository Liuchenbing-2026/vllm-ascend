...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight', 'weight_scale', 'x_scale', 'weight_assistance_matrix', 'group_list']
...
def _build_args(x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__, group_list_in__, y_out_, y_scale_out_, is_enable_weight_assistance_matrix, dequant_mode, limited):
    __inputs__ = []
    for arg in [x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__, group_list_in__]:
        if arg != None:
    ...
    for arg in [y_out_, y_scale_out_]:
        if arg != None:
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if is_enable_weight_assistance_matrix != None:
        attr = {}
        attr["name"] = "is_enable_weight_assistance_matrix"
        attr["dtype"] = "bool"
        attr["value"] = is_enable_weight_assistance_matrix
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def grouped_matmul_swiglu_quant(x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__=None, group_list_in__=None, y_out_=None, y_scale_out_=None, is_enable_weight_assistance_matrix=True, dequant_mode=0, limited=0, kernel_name="grouped_matmul_swiglu_quant", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__, group_list_in__, y_out_, y_scale_out_, is_enable_weight_assistance_matrix, dequant_mode, limited)
    ...
    msg = "start compile Ascend C Operator GroupedMatmulSwigluQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__, group_list_in__], origin_outputs = [y_out_, y_scale_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__=None, group_list_in__=None, y_out_=None, y_scale_out_=None, is_enable_weight_assistance_matrix=True, dequant_mode=0, limited=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__, group_list_in__, y_out_, y_scale_out_, is_enable_weight_assistance_matrix, dequant_mode, limited)
    result = check_op_cap("op_select_format", "GroupedMatmulSwigluQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__=None, group_list_in__=None, y_out_=None, y_scale_out_=None, is_enable_weight_assistance_matrix=True, dequant_mode=0, limited=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, weight_scale_in__, x_scale_in__, weight_assistance_matrix_in__, group_list_in__, y_out_, y_scale_out_, is_enable_weight_assistance_matrix, dequant_mode, limited)
