...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['k', 'w', 'u', 'g', 'inital_state', 'cu_seqlens', 'chunk_indices']
    output_names = ['h', 'v_new', 'final_state']
...
def _build_args(k_in__, w_in__, u_in__, g_in__, inital_state_in__, cu_seqlens_in__, chunk_indices_in__, h_out_, v_new_out_, final_state_out_, output_final_state, chunk_size, inital_state_stride0):
    __inputs__ = []
    for arg in [k_in__, w_in__, u_in__, g_in__, inital_state_in__, cu_seqlens_in__, chunk_indices_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [h_out_, v_new_out_, final_state_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if output_final_state != None:
        attr = {}
        attr["name"] = "output_final_state"
        attr["dtype"] = "bool"
        attr["value"] = output_final_state
    ...
    if chunk_size != None:
        ...
        __attrs__.append(attr)
    if inital_state_stride0 != None:
        attr = {}
        attr["name"] = "inital_state_stride0"
        attr["dtype"] = "int"
        attr["value"] = inital_state_stride0
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ChunkGatedDeltaRuleFwdH", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def chunk_gated_delta_rule_fwd_h(k_in__, w_in__, u_in__, g_in__, inital_state_in__=None, cu_seqlens_in__=None, chunk_indices_in__=None, h_out_=None, v_new_out_=None, final_state_out_=None, output_final_state=False, chunk_size=0, inital_state_stride0=0, kernel_name="chunk_gated_delta_rule_fwd_h", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(k_in__, w_in__, u_in__, g_in__, inital_state_in__, cu_seqlens_in__, chunk_indices_in__, h_out_, v_new_out_, final_state_out_, output_final_state, chunk_size, inital_state_stride0)
    ...
    custom_compile_options = {'__ALLSOC__': ['--cce-auto-sync=off', '-Wno-deprecated-declarations', '-Werror', '-I/tmp/vllm-ascend/csrc/third_party/catlass/include', '-I/tmp/vllm-ascend/csrc/moe/common']},
    ...
    origin_func_name = "chunk_gated_delta_rule_fwd_h"
    ascendc_src_dir_ex = "chunk_gated_delta_rule_fwd_h"
    ascendc_src_dir = "chunk_gated_delta_rule_fwd_h"
    ascendc_src_file = "chunk_gated_delta_rule_fwd_h.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator ChunkGatedDeltaRuleFwdH, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ChunkGatedDeltaRuleFwdH"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[k_in__, w_in__, u_in__, g_in__, inital_state_in__, cu_seqlens_in__, chunk_indices_in__], origin_outputs = [h_out_, v_new_out_, final_state_out_],\
    ...
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {5: 'optional', 6: 'optional'}})

def op_select_format(k_in__, w_in__, u_in__, g_in__, inital_state_in__=None, cu_seqlens_in__=None, chunk_indices_in__=None, h_out_=None, v_new_out_=None, final_state_out_=None, output_final_state=False, chunk_size=0, inital_state_stride0=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(k_in__, w_in__, u_in__, g_in__, inital_state_in__, cu_seqlens_in__, chunk_indices_in__, h_out_, v_new_out_, final_state_out_, output_final_state, chunk_size, inital_state_stride0)
    result = check_op_cap("op_select_format", "ChunkGatedDeltaRuleFwdH", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(k_in__, w_in__, u_in__, g_in__, inital_state_in__=None, cu_seqlens_in__=None, chunk_indices_in__=None, h_out_=None, v_new_out_=None, final_state_out_=None, output_final_state=False, chunk_size=0, inital_state_stride0=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(k_in__, w_in__, u_in__, g_in__, inital_state_in__, cu_seqlens_in__, chunk_indices_in__, h_out_, v_new_out_, final_state_out_, output_final_state, chunk_size, inital_state_stride0)
    result = check_op_cap("get_op_specific_info", "ChunkGatedDeltaRuleFwdH", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
