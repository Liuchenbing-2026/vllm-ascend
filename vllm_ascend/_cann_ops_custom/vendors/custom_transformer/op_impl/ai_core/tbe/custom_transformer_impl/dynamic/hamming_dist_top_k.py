...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['query', 'key_compressed', 'k', 'seq_len', 'chunk_size', 'key_block_table', 'indices_in', 'key_compressed_rope', 'mask']
...
def _build_args(query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__, mask_in__, indices_out_, max_seq_len, sink, recent, support_offload):
    __inputs__ = []
    for arg in [query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__, mask_in__]:
        if arg != None:
    ...
    if max_seq_len != None:
        attr = {}
        attr["name"] = "max_seq_len"
        attr["dtype"] = "int"
        attr["value"] = max_seq_len
    ...
    if recent != None:
        ...
        attr["value"] = recent
        __attrs__.append(attr)
    if support_offload != None:
        attr = {}
        attr["name"] = "support_offload"
        attr["dtype"] = "int"
        attr["value"] = support_offload
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("HammingDistTopK", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def hamming_dist_top_k(query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__=None, mask_in__=None, indices_out_=None, max_seq_len=0, sink=0, recent=0, support_offload=0, kernel_name="hamming_dist_top_k", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__, mask_in__, indices_out_, max_seq_len, sink, recent, support_offload)
    ...
    custom_compile_options = {'__ALLSOC__': ['--cce-auto-sync=on', '-Wno-deprecated-declarations', '-Werror', '-mllvm', '-cce-aicore-hoist-movemask=false']},
    ...
    origin_func_name = "hamming_dist_top_k"
    ascendc_src_dir_ex = "hamming_dist_top_k"
    ascendc_src_dir = "hamming_dist_top_k"
    ascendc_src_file = "hamming_dist_top_k.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator HammingDistTopK, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "HammingDistTopK"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__, mask_in__], origin_outputs = [indices_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__=None, mask_in__=None, indices_out_=None, max_seq_len=0, sink=0, recent=0, support_offload=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__, mask_in__, indices_out_, max_seq_len, sink, recent, support_offload)
    result = check_op_cap("op_select_format", "HammingDistTopK", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__=None, mask_in__=None, indices_out_=None, max_seq_len=0, sink=0, recent=0, support_offload=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_compressed_in__, k_in__, seq_len_in__, chunk_size_in__, key_block_table_in__, indices_in_in__, key_compressed_rope_in__, mask_in__, indices_out_, max_seq_len, sink, recent, support_offload)
    result = check_op_cap("get_op_specific_info", "HammingDistTopK", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
