...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['target_token_ids', 'target_positions', 'next_token_ids', 'query_start_loc', 'query_end_loc']
    output_names = ['out_input_ids', 'out_positions', 'out_is_rejected_token_mask', 'out_is_masked_token_mask', 'out_new_token_indices', 'out_hidden_state_mapping']
...
def _build_args(target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__, out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_, padding_token_id, parallel_drafting_token_id, num_padding_slots_per_request, shift_input_ids, total_input_tokens):
    __inputs__ = []
    for arg in [target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if padding_token_id != None:
        attr = {}
        attr["name"] = "padding_token_id"
        attr["dtype"] = "int"
        attr["value"] = padding_token_id
        __attrs__.append(attr)
    if parallel_drafting_token_id != None:
        attr = {}
        attr["name"] = "parallel_drafting_token_id"
        attr["dtype"] = "int"
        attr["value"] = parallel_drafting_token_id
        __attrs__.append(attr)
    if num_padding_slots_per_request != None:
        attr = {}
        attr["name"] = "num_padding_slots_per_request"
        attr["dtype"] = "int"
        attr["value"] = num_padding_slots_per_request
        __attrs__.append(attr)
    if shift_input_ids != None:
        attr = {}
        attr["name"] = "shift_input_ids"
        attr["dtype"] = "bool"
        attr["value"] = shift_input_ids
        __attrs__.append(attr)
    if total_input_tokens != None:
        attr = {}
        attr["name"] = "total_input_tokens"
        attr["dtype"] = "int"
        attr["value"] = total_input_tokens
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("CopyAndExpandEagleInputs", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def copy_and_expand_eagle_inputs(target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__, out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_, padding_token_id, parallel_drafting_token_id, num_padding_slots_per_request, shift_input_ids, total_input_tokens, kernel_name="copy_and_expand_eagle_inputs", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__, out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_, padding_token_id, parallel_drafting_token_id, num_padding_slots_per_request, shift_input_ids, total_input_tokens)
    ...
    origin_func_name = "copy_and_expand_eagle_inputs"
    ascendc_src_dir_ex = "copy_and_expand_eagle_inputs"
    ascendc_src_dir = "copy_and_expand_eagle_inputs"
    ascendc_src_file = "copy_and_expand_eagle_inputs.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator CopyAndExpandEagleInputs, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "CopyAndExpandEagleInputs"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__], origin_outputs = [out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__, out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_, padding_token_id, parallel_drafting_token_id, num_padding_slots_per_request, shift_input_ids, total_input_tokens, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__, out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_, padding_token_id, parallel_drafting_token_id, num_padding_slots_per_request, shift_input_ids, total_input_tokens)
    result = check_op_cap("op_select_format", "CopyAndExpandEagleInputs", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__, out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_, padding_token_id, parallel_drafting_token_id, num_padding_slots_per_request, shift_input_ids, total_input_tokens, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(target_token_ids_in__, target_positions_in__, next_token_ids_in__, query_start_loc_in__, query_end_loc_in__, out_input_ids_out_, out_positions_out_, out_is_rejected_token_mask_out_, out_is_masked_token_mask_out_, out_new_token_indices_out_, out_hidden_state_mapping_out_, padding_token_id, parallel_drafting_token_id, num_padding_slots_per_request, shift_input_ids, total_input_tokens)
    result = check_op_cap("get_op_specific_info", "CopyAndExpandEagleInputs", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
