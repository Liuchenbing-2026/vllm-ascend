...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'wkv', 'wgate', 'state_cache', 'ape', 'norm_weight', 'rope_sin', 'rope_cos', 'state_block_table', 'cu_seqlens', 'seqused', 'start_pos']
    output_names = ['cmp_kv', 'state_cache']
...
def _build_args(x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__, cu_seqlens_in__, seqused_in__, start_pos_in__, cmp_kv_out_, state_cache_out_, rope_head_dim, cmp_ratio, coff, norm_eps, rotary_mode, cache_mode, state_cache_stride_dim0):
    __inputs__ = []
    for arg in [x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__, cu_seqlens_in__, seqused_in__, start_pos_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [cmp_kv_out_, state_cache_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if rope_head_dim != None:
        attr = {}
        attr["name"] = "rope_head_dim"
        attr["dtype"] = "int"
        attr["value"] = rope_head_dim
    ...
    if rotary_mode != None:
        attr = {}
        attr["name"] = "rotary_mode"
        attr["dtype"] = "int"
        attr["value"] = rotary_mode
        __attrs__.append(attr)
    if cache_mode != None:
        attr = {}
        ...
        __attrs__.append(attr)
    if state_cache_stride_dim0 != None:
        attr = {}
        attr["name"] = "state_cache_stride_dim0"
        attr["dtype"] = "int"
        attr["value"] = state_cache_stride_dim0
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("Compressor", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def compressor(x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__=None, cu_seqlens_in__=None, seqused_in__=None, start_pos_in__=None, cmp_kv_out_=None, state_cache_out_=None, rope_head_dim=0, cmp_ratio=0, coff=1, norm_eps=1e-06, rotary_mode=1, cache_mode=1, state_cache_stride_dim0=0, kernel_name="compressor", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__, cu_seqlens_in__, seqused_in__, start_pos_in__, cmp_kv_out_, state_cache_out_, rope_head_dim, cmp_ratio, coff, norm_eps, rotary_mode, cache_mode, state_cache_stride_dim0)
    ...
    origin_func_name = "compressor"
    ascendc_src_dir_ex = "compressor"
    ascendc_src_dir = "compressor"
    ascendc_src_file = "compressor.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator Compressor, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Compressor"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__, cu_seqlens_in__, seqused_in__, start_pos_in__], origin_outputs = [cmp_kv_out_, state_cache_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__=None, cu_seqlens_in__=None, seqused_in__=None, start_pos_in__=None, cmp_kv_out_=None, state_cache_out_=None, rope_head_dim=0, cmp_ratio=0, coff=1, norm_eps=1e-06, rotary_mode=1, cache_mode=1, state_cache_stride_dim0=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__, cu_seqlens_in__, seqused_in__, start_pos_in__, cmp_kv_out_, state_cache_out_, rope_head_dim, cmp_ratio, coff, norm_eps, rotary_mode, cache_mode, state_cache_stride_dim0)
    result = check_op_cap("op_select_format", "Compressor", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__=None, cu_seqlens_in__=None, seqused_in__=None, start_pos_in__=None, cmp_kv_out_=None, state_cache_out_=None, rope_head_dim=0, cmp_ratio=0, coff=1, norm_eps=1e-06, rotary_mode=1, cache_mode=1, state_cache_stride_dim0=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, wkv_in__, wgate_in__, state_cache_in__, ape_in__, norm_weight_in__, rope_sin_in__, rope_cos_in__, state_block_table_in__, cu_seqlens_in__, seqused_in__, start_pos_in__, cmp_kv_out_, state_cache_out_, rope_head_dim, cmp_ratio, coff, norm_eps, rotary_mode, cache_mode, state_cache_stride_dim0)
    result = check_op_cap("get_op_specific_info", "Compressor", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
