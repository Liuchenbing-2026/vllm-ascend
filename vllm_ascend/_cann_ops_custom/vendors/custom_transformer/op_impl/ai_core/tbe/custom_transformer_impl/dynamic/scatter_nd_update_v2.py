...
def _build_args(var_in__, indices_in__, updates_in__, var_out_, strides, use_locking):
    __inputs__ = []
...
@tbe_register.register_operator("ScatterNdUpdateV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def scatter_nd_update_v2(var_in__, indices_in__, updates_in__, var_out_, strides, use_locking=False, kernel_name="scatter_nd_update_v2", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, strides, use_locking)
    ...
    origin_func_name = "scatter_nd_update_v2"
    ascendc_src_dir_ex = "scatter_nd_update_v2"
    ascendc_src_dir = "scatter_nd_update_v2"
    ascendc_src_file = "scatter_nd_update_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator ScatterNdUpdateV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
...
def op_select_format(var_in__, indices_in__, updates_in__, var_out_, strides, use_locking=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, strides, use_locking)
    result = check_op_cap("op_select_format", "ScatterNdUpdateV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, indices_in__, updates_in__, var_out_, strides, use_locking=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, strides, use_locking)
    result = check_op_cap("get_op_specific_info", "ScatterNdUpdateV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
