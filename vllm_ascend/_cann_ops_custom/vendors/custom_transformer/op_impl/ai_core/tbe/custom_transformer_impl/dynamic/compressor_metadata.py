...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['ropeCos', 'ropeSin', 'cuSeqlens', 'startPos', 'kvBlockTable']
    output_names = ['compressCos', 'compressSin', 'slotMapping']
...
def _build_args(ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__, compressCos_out_, compressSin_out_, slotMapping_out_, kvBlockSize, slotMappingFormat, cmpRatio, actualNumReqs):
    __inputs__ = []
    for arg in [ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [compressCos_out_, compressSin_out_, slotMapping_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if kvBlockSize != None:
        attr = {}
        attr["name"] = "kvBlockSize"
        attr["dtype"] = "int"
        attr["value"] = kvBlockSize
        __attrs__.append(attr)
    if slotMappingFormat != None:
        attr = {}
        attr["name"] = "slotMappingFormat"
        attr["dtype"] = "int"
        attr["value"] = slotMappingFormat
        __attrs__.append(attr)
    if cmpRatio != None:
        attr = {}
        attr["name"] = "cmpRatio"
        ...
        attr["value"] = cmpRatio
        __attrs__.append(attr)
    if actualNumReqs != None:
        attr = {}
        attr["name"] = "actualNumReqs"
        attr["dtype"] = "int"
        attr["value"] = actualNumReqs
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("CompressorMetadata", trans_bool_to_s8=False)
...
def compressor_metadata(ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__, compressCos_out_, compressSin_out_, slotMapping_out_, kvBlockSize, slotMappingFormat, cmpRatio, actualNumReqs, kernel_name="compressor_metadata", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__, compressCos_out_, compressSin_out_, slotMapping_out_, kvBlockSize, slotMappingFormat, cmpRatio, actualNumReqs)
    ...
    origin_func_name = "compressor_metadata"
    ascendc_src_dir_ex = "compressor_metadata"
    ascendc_src_dir = "compressor_metadata"
    ascendc_src_file = "compressor_metadata.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator CompressorMetadata, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "CompressorMetadata"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__], origin_outputs = [compressCos_out_, compressSin_out_, slotMapping_out_],\
...
def op_select_format(ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__, compressCos_out_, compressSin_out_, slotMapping_out_, kvBlockSize, slotMappingFormat, cmpRatio, actualNumReqs, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__, compressCos_out_, compressSin_out_, slotMapping_out_, kvBlockSize, slotMappingFormat, cmpRatio, actualNumReqs)
    result = check_op_cap("op_select_format", "CompressorMetadata", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__, compressCos_out_, compressSin_out_, slotMapping_out_, kvBlockSize, slotMappingFormat, cmpRatio, actualNumReqs, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(ropeCos_in__, ropeSin_in__, cuSeqlens_in__, startPos_in__, kvBlockTable_in__, compressCos_out_, compressSin_out_, slotMapping_out_, kvBlockSize, slotMappingFormat, cmpRatio, actualNumReqs)
    result = check_op_cap("get_op_specific_info", "CompressorMetadata", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
