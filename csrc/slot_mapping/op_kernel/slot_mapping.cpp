 ...
 *
 * Block-table access is a data-dependent gather: each token reads exactly ONE
 * entry blockTable[req, pos/blockSize]. We therefore read those entries
 * directly from GM (gmBt_.GetValue) instead of staging the whole table in UB.
 * Staging the full table (numReqs * stride int32) was both wasteful (every
 * core re-loaded the entire table to use <=16 entries) and a hard correctness
 * bug: for large batch x context numReqs*stride*4 exceeds UB capacity and the
 * staging DataCopy faults ("MTE write address out of range" vector-core
 * exception). GM gather removes the UB ceiling and the redundant traffic.
 *
 * Pure-padding cores (posStart >= numTokens, validInTile == 0) only need to
 * write padId; they skip the qsl/positions loads and the lookup entirely.
 *
...
public:
        ...
        const SlotMappingTilingData* td)
    {
        ...
        // ---------- validInTile_ (how many of myNumPos_ are real tokens) -------
        ...
        // Output buffers are always needed (every core writes its tile).
        uint32_t slotBytes = AlignUp(static_cast<uint32_t>(myNumPos_) * sizeof(int64_t), ONE_BLK_SIZE);
        pipe_.InitBuffer(bufSlot_, slotBytes);
        ...
        // Pure-padding core: no inputs needed, Process() only writes padId.
        if (validInTile_ <= 0) {
            return;
        }
        ...
        uint32_t posBytes = AlignUp(static_cast<uint32_t>(validInTile_) * sizeof(int64_t), ONE_BLK_SIZE);
        pipe_.InitBuffer(bufPos_, posBytes);
        int32_t posAligned = (validInTile_ + kInt64PerBlk - 1) / kInt64PerBlk * kInt64PerBlk;

        // ---------- Load qsl + positions on MTE2 (block table is gathered from
        // GM in Process(), so it is NOT staged here) ----------
        ...
        // Single barrier waits for the MTE2 loads before Process() reads them
        // via scalar GetValue.
    ...
    __aicore__ inline void Process()
    {
        ...
        if (validInTile_ > 0) {
            ...
            if (fastPath) {
                ...
                for (int32_t i = 0; i < validInTile_; ++i) {
                    ...
                    // Gather the single needed block-table entry directly from GM.
                    int32_t blockNum = gmBt_.GetValue(
        ...
        // 2^31; the kernel contract is int32 output. myNumPos_ <= tilePerCore.
        ...
        // One DataCopyPad writes the whole tile back to GM (int32).
...
private:
    ...
    // UB Buffers (block table is gathered from GM, so no bufBt_)
    TPipe pipe_;
