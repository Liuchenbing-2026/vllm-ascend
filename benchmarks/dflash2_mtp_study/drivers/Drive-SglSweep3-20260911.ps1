...
$log   = Join-Path $work 'drive-sgl3-20260911.log'
...
$steps = @(
  @{ id='20260911al'; script='benchserve_start_mtp3_sgl_m18_20260911d.sh';   marker='BENCHSERVE_START_READY'; wait=35 },
  @{ id='20260911am'; script='benchserve_bench_mtp3_sgl_m18_20260911d.sh';   marker='BENCHSGL_PASS';          wait=30 },
  @{ id='20260911an'; script='benchserve_start_nospec_sgl_m18_20260911d.sh'; marker='BENCHSERVE_START_READY'; wait=35 },
  @{ id='20260911ao'; script='benchserve_bench_nospec_sgl_m18_20260911d.sh'; marker='BENCHSGL_PASS';          wait=30 }
)

Say 'waiting on 20260911ak (k7-eos) to release the cards'
$prev = Wait-Task '20260911ak' 25
if ($null -eq $prev) { Say 'ABORT: 20260911ak never reached a terminal state'; exit 1 }
Say ('ak settled: ' + (($prev.text -split "`n" | Where-Object { $_ -match 'BENCHSGL|released' } | Select-Object -Last 3) -join ' | '))
