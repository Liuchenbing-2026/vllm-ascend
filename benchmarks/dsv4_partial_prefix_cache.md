...
Both only act on prompts that get **zero** whole-block hits (i.e. <16K at
...
```bash
# PC ON, partial active (short prompts hit; no extra env needed):
...
- No env var or tuning is required: the partial path always uses every hit and
  is on whenever `--enable-prefix-caching` is set. Batched + overlapped copy
  makes short hits profitable, so there is no skip knob to configure.
