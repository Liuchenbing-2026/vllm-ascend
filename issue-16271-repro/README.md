...
#### 接受率的两个口径：`vllm bench` 自己报的比 pooled 保守（得说清楚）

上面那列 `pooled accept_len` 是 `harness/pooled_acc.py` 扫整个 serve 日志算的，
**包括了并发 1 的 probe 段**。`vllm bench serve` 自己就报接受率，且只覆盖 measure 段：

| 臂 | 吞吐 tok/s | 中位 ITL ms | bench 接受率 % | **bench accept_len** | pooled accept_len |
|---|---|---|---|---|---|
| base1 / base2 | 132.14 / 131.33 | 75.60 / 75.11 | 7.06 / 6.61 | **1.56 / 1.53** | 1.6340 / 1.5893 |
| c01 / c02 | 176.90 / 184.04 | 50.21 / 50.03 | 5.93 / 6.54 | **1.47 / 1.52** | 1.5149 / 1.5472 |
| **mf1 / mf2** | **185.17 / 193.02** | **49.86 / 50.19** | 6.99 / 7.51 | **1.56 / 1.60** | 1.6277 / 1.6547 |

两个口径对接受率的结论方向一致，**但分开程度差很多**：
- pooled 口径上 `c0` 比 base 低 5.00%；
- **bench-only 口径上 `c0`（1.47 / 1.52）与 base（1.53 / 1.56）只是擦边不重叠**，远没有 5% 那么大。

差别的来源是 probe 段：那一段并发 1、接受长度本来就高（base 2.5864 vs `c0` 2.2710），
**绝对差大得多**，混进 pooled 里就把区分度放大了。
两个口径上都成立的是：**`mf` 的两个实例都在 base 顶部及以上，且都高于 `c0` 的两个**。
要把 `c0` 的代价说成“5%”就必须标明是 pooled 口径；在 c=8 的 bench 工负载上它更接近 **2~4%**。
...
### 5.4 正式形态的端到端复跑（`harness/leg_clean.sh`，三臂 `cl1 c0x cl2`）

§5.2 那张表是 monkeypatch 形态（`harness/maskfix_patch.py`）跑的。
**本分支的正式代码自己跑了一遍**，同样是 `vllm bench serve`（random，in 512 / out 256，
96 prompts，c=8，`--ignore-eos --seed 1234`），臂序 `cl / c0 / cl` 让漂移不会只落在一个臂上。

| 臂 | 吞吐 tok/s | 平均 TPOT ms | 中位 ITL ms | bench 接受率 % | **bench accept_len** | 接受 token 数 |
|---|---|---|---|---|---|---|
| **`cl1`（本方案）** | **184.12** | 39.81 | 50.42 | 7.40 | **1.59** | 9141 |
| `c0x`（只喂上界） | 173.41 | 42.54 | 50.31 | 5.76 | **1.46** | 7737 |
| **`cl2`（本方案）** | **188.25** | 39.92 | 50.41 | 7.15 | **1.57** | 8933 |

**本方案 vs `c0`（同一条 leg 内的对照）**：
...
| 平均 TPOT | −6.4% | −6.2% |
| accept_len | **+8.9%** | **+7.5%** |
| 接受 token 数 | +18.1% | +15.5% |
| 中位 ITL | +0.2% | +0.2%（三臂一样） |

两个 `cl` 实例在吞吐和接受率上**都高于**那个 `c0` 实例。
**中位 ITL 三臂完全一致（50.31–50.42 ms）**——这正合预期：去掉阻塞同步的是
`c0` 就已经做到的事，掩码不再动下发路径；本方案赚的是**接受率换来的吞吐**。

对照上一条 leg 的 base（`vllm bench` 口径：132.14 / 131.33 tok/s，
中位 ITL 75.60 / 75.11 ms，accept_len 1.56 / 1.53）：正式形态 **吞吐 +40%、ITL −33%、
accept_len 1.59 / 1.57 落在 base 之上**。⚠️ **这是跨 leg 比较**——base 不在这条 leg 里，
而方差单位是重启，所以这一条只当方向看，同 leg 内的对照只有 `c0`。

⚠️ 另一条：这条 leg 是 2 个 `cl` 对 **1 个** `c0`。两个 `cl` 把那个 `c0` 夹在中间下方，
方向清楚，但仍然给不了显著性。

#### 两条 leg 合起来看：7 个实例，三个指标上 `c0` 与本方案完全不重叠

两条 leg 的 `vllm bench serve`（measure 段）放在一起，本方案共 4 个实例、`c0` 共 3 个：

| | 吞吐 tok/s | 平均 TPOT ms | **bench accept_len** |
|---|---|---|---|
| base（精确长度，2 个） | 132.14 / 131.33 | 55.88 / 56.58 | 1.56 / 1.53 |
| `c0`（只喂上界，3 个） | 173.41 / 176.90 / 184.04 | 42.54 / 42.35 / 40.45 | 1.46 / 1.47 / 1.52 |
| **本方案（4 个）** | **184.12 / 185.17 / 188.25 / 193.02** | **39.81 / 39.58 / 39.92 / 38.97** | **1.56 / 1.57 / 1.59 / 1.60** |

- **accept_len**：`c0` 最大 1.52 < 本方案最小 1.56 —— 七个实例**完全分开**；
- **吞吐**：`c0` 最大 184.04 < 本方案最小 184.12 —— 也分开，但只差 0.08，擦边；
- **平均 TPOT**：`c0` 最小 40.45 > 本方案最大 39.92 —— 分开；
- **base 的 1.53 / 1.56 与本方案的下端重叠** ⇒ 接受率是**全额拿回**，不是拿回一部分。

⚠️ 这是**跨 leg 汇总**，方差单位是重启，两条 leg 之间还隔着别人的负载。
所以它说明的是"方向在 7 个独立实例上没有例外"，不是显著性检验。

#### probe 段的相等判据在新 leg 再现

并发 1、贪婪、`HCCL_DETERMINISTIC=true`、固定 12 条 prompt ⇒ 计数器确定，
可以直接比相等（`harness/probe_acc.py`，截到第 13 个 completion 响应之前）：

| 臂 | Accepted | Drafted | probe accept_len |
|---|---|---|---|
| `base1`（精确长度） | 1883 | 9496 | 2.5864 |
| `mf1`（monkeypatch） | 1883 | 9496 | 2.5864 |
| **`cl1` / `cl2`（正式形态）** | **1883** | **9496** | **2.5864** |
| `c01` / `c0x`（只喂上界） | 1721 | 10832 | 2.2710 |

**掩码路径四次独立重启（`mf1` `mf2` `cl1` `cl2`）计数器逐位相同，且与精确长度那一路相同**；
`c0` 三次独立重启也逐位相同，但落在另一档。
`cl` 臂的 `VLLM_ASCEND_DSPARK_APPROX_DRAFT_KV` 一直是 1、只切 `..._DEVICE_MASK`，
所以掩码若整条回退，`cl` 会**精确**等于 `c0` 的 1721/10832 —— 它没有。

**输出逐字节相同**：`cl1` / `cl2` / `c0x` 与 monkeypatch leg 的 `mf1` 同一个
sha `df34ea140872348c`。

**第一次尝试被打断**（保留记录）：19:35 卡空出来自动起了这条 leg，`cl1` 起服、probe 跑完，
**19:42 容器被 SIGKILL**（`exited 137`，`OOMKilled=false`，宿主内存 42/1005 GB ⇒ 外部 kill），
8 张卡随即被别人的 TP8 任务占满，`vllm bench` 一条请求都没发出去
（`logs/clean_partial.out`、`logs/probe_acc.out`）。22:16 卡再次空出来后重跑，就是上面的结果。
⇒ **共享机器上把清理只放在 `trap cleanup EXIT` 里是不够的**，容器被 kill 就不执行；
那次残留的 4 个改动 + 5 个备份是事后手工还原的。
...
5. **上面的端到端数是用 monkeypatch 形态跑的**（`harness/maskfix_patch.py`），
   ...
   **已自己跑过完整的 `vllm bench serve`（→ §5.4）**：同 leg 内对 `c0` 是
   吞吐 +6.2%/+8.6%、accept_len +8.9%/+7.5%，probe 段计数器与精确路径逐位相同。
   ❗但那条 leg 是 **2 个 `cl` 对 1 个 `c0`、且没有 base 臂**，与 base 的对比是跨 leg 的。
...
| `harness/leg_clean.sh` | 正式形态的复跑验证（§5.4） |
| `harness/probe_acc.py` | 把 serve 日志截到 probe 阶段再汇总接受率（§5.4 的尺子） |
| `logs/mask.out` | 六臂端到端 A/B 原始日志 |
| `logs/clean.out` | 正式形态三臂 leg 的原始日志（§5.4） |
| `logs/clean_partial.out` | 第一次尝试，跑到 `cl1` probe 就被 SIGKILL |
