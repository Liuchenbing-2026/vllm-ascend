...
| **🏆 §8.7.6 正式形态已自己跑完 `vllm bench`（2026-09-13）** | 第一次复跑跑到 probe 就被别人的 8 卡任务 SIGKILL（`exited 137`，非 OOM）；22:16 卡再空出来后三臂 `cl1 c0x cl2` 跑完。**同 leg 内对 `c0`：吞吐 +6.2%/+8.6%（184.12/188.25 vs 173.41）、平均 TPOT −6.4%/−6.2%、accept_len +8.9%/+7.5%（1.59/1.57 vs 1.46）**；中位 ITL 三臂一样（50.31–50.42）——去同步是 `c0` 就做到的，本方案赚的是接受率换来的吞吐。**两条 leg 合计 7 个实例，`c0`（3）与本方案（4）在 accept_len / 吞吐 / TPOT 三个指标上全部不重叠**（accept_len：`c0` max 1.52 < 本方案 min 1.56），而 base（1.53/1.56）与本方案下端重叠 ⇒ 接受率全额拿回。probe 段相等判据：掩码路径**四次独立重启计数器逐位相同且等于精确路径**（1883/9496），`c0` 三次都是 1721/10832 |
...
#### ⚠️ 上面那列 `pooled accept_len` 把 probe 段混进来了，区分度被放大

`pooled_acc.py` 扫的是整个 serve 日志，里面既有 bench（c=8）也有 probe（c=1）。
`vllm bench serve` 自己也报接受率，且只覆盖 measure 段：

| 臂 | bench 接受率 % | **bench accept_len** | pooled accept_len |
|---|---|---|---|
| base1 / base2 | 7.06 / 6.61 | **1.56 / 1.53** | 1.6340 / 1.5893 |
| c01 / c02 | 5.93 / 6.54 | **1.47 / 1.52** | 1.5149 / 1.5472 |
| mf1 / mf2 | 6.99 / 7.51 | **1.56 / 1.60** | 1.6277 / 1.6547 |

方向一致，但**分开程度差很多**：pooled 上 `c0` 比 base 低 5.00%，
bench-only 上 `c0`（1.47/1.52）与 base（1.53/1.56）只是擦边不重叠。
原因是 probe 段并发 1、接受长度本来就高（base 2.5864 vs `c0` 2.2710，绝对差大得多），
混进去就把区分度放大了。⇒ **说 `c0` 的代价是“5%”必须标明 pooled 口径**；
c=8 的 bench 工负载上更接近 2~4%。两个口径都成立的是
`mf` 两个实例都在 base 顶部及以上、且都高于 `c0` 两个。
...
### 8.7.6 正式形态的端到端复跑：`vllm bench` 跑成了，同 leg 内对 `c0` 全面赢（2026-09-13）

§8.7.5 的数出自 monkeypatch 形态。整理成正式补丁后必须自己再跑一遍——
**"整理完没跑过"正是这次调查反复踩的那类失败**（编译过、服务起得来、
然后那个臂安静地退化成 do-nothing 臂）。

#### 第一次尝试：跑到 probe 就被别人的 8 卡任务打断

`scripts/wait_and_launch.sh` 在机器上轮询等卡（等待放机器上跑，本地长轮询会被内存回收杀掉），
19:35 卡空出来自动起了 `scripts/leg_clean.sh`。`cl1` 起服 342 s、probe 12 条跑完，
**19:42:36 容器被 SIGKILL**（`docker inspect` → `exited 137`、`OOMKilled=false`，
宿主内存 42/1005 GB ⇒ 外部 kill），随即 8 卡被别人的 TP8 `VLLMWorker_TP` 占满。
`bench_cl1_warm.log` 全文只有 5 行插件加载日志——**`vllm bench` 一条请求都没发出去**。

⇒ **共享机器上把清理只放在 `trap cleanup EXIT` 里是不够的**：容器被 kill 不执行 trap，
树被留脏（4 个 M + 5 个 `.nt-*orig`），事后手工还原。

#### 第二次：22:16 卡再空出来，三臂 `cl1 c0x cl2` 跑完

同样的 `vllm bench serve`（random，in 512 / out 256，96 prompts，c=8，`--ignore-eos --seed 1234`），
臂序 `cl / c0 / cl`：

| 臂 | 吞吐 tok/s | 平均 TPOT ms | 中位 ITL ms | 接受率 % | **accept_len** | 接受 token 数 | pooled accept_len |
|---|---|---|---|---|---|---|---|
| **`cl1`** | **184.12** | 39.81 | 50.42 | 7.40 | **1.59** | 9141 | 1.6267 |
| `c0x` | 173.41 | 42.54 | 50.31 | 5.76 | **1.46** | 7737 | 1.5115 |
| **`cl2`** | **188.25** | 39.92 | 50.41 | 7.15 | **1.57** | 8933 | 1.5757 |

同 leg 内对 `c0`：吞吐 **+6.2% / +8.6%**，平均 TPOT **−6.4% / −6.2%**，
accept_len **+8.9% / +7.5%**，接受 token 数 +18.1% / +15.5%。

**中位 ITL 三臂完全一致（50.31–50.42 ms）**——正合预期：去掉阻塞同步是 `c0` 就已经做到的，
掩码不再动下发路径，本方案赚的是**接受率换来的吞吐**（同样的 ITL 下每步多接受一点）。

#### 两条 leg 合起来：7 个实例，`c0` 与本方案三个指标上都不重叠

| | 吞吐 tok/s | 平均 TPOT ms | **accept_len** |
|---|---|---|---|
| base（2 个） | 132.14 / 131.33 | 55.88 / 56.58 | 1.56 / 1.53 |
| `c0`（3 个） | 173.41 / 176.90 / 184.04 | 42.54 / 42.35 / 40.45 | 1.46 / 1.47 / 1.52 |
| **本方案（4 个）** | **184.12 / 185.17 / 188.25 / 193.02** | **39.81 / 39.58 / 39.92 / 38.97** | **1.56 / 1.57 / 1.59 / 1.60** |

- accept_len：`c0` 最大 1.52 **<** 本方案最小 1.56，完全分开；
- 吞吐：`c0` 最大 184.04 **<** 本方案最小 184.12，分开但只差 0.08（擦边）；
- 平均 TPOT：`c0` 最小 40.45 **>** 本方案最大 39.92，分开；
- **base 的 1.53 / 1.56 与本方案下端重叠** ⇒ 接受率全额拿回。

⚠️ 跨 leg 汇总，方差单位是重启，中间还隔着别人的负载 ⇒ 说明的是
"方向在 7 个独立实例上没有例外"，不是显著性检验。

#### probe 段的相等判据（比上面的区间论证更硬）

并发 1、贪婪、`HCCL_DETERMINISTIC=true`、固定 12 条 prompt ⇒ 计数器逐位确定，
可以直接比**相等**。`scripts/probe_acc.py` 把每条 serve 日志截到第 13 个
completion 响应之前（probe 恰好发 12 条）：

| 臂 | Accepted | Drafted | probe accept_len |
|---|---|---|---|
| `base1` / `base2` | 1883 | 9496 | 2.5864 |
| `mf1` / `mf2` / **`cl1`** / **`cl2`** | **1883** | **9496** | **2.5864** |
| `c01` / `c02` / `c0x` | 1721 | 10832 | 2.2710 |

- **掩码路径四次独立重启计数器逐位相同，且与精确长度那一路相同**；
- `c0` 三次独立重启也逐位相同，落在另一档（少接受 162、多起草 1336）⇒ **尺子有区分力**；
- `cl` 臂的 `APPROX_DRAFT_KV` 一直是 1、只切 `DEVICE_MASK`，
  掩码若整条回退 `cl` 会**精确**等于 `c0` ⇒ **排除静默空转**。

**输出逐字节相同**：`cl1` / `cl2` / `c0x` 与 monkeypatch leg 的 `mf1`
同一个 sha `df34ea140872348c`。

#### 还要留意的

1. 这条 leg 是 **2 个 `cl` 对 1 个 `c0`**（臂序 `cl/c0/cl` 防漂移），且**没有 base 臂**——
   与 base 的对比是跨 leg 的。
2. §8.7.2 的"n=64 + 滑窗，通用模板贵 50.8%"在 c=8 下依然没变成端到端回归，
   大并发仍未测。
3. 接受率贴地板（random 集 `ratio≈0.07`），健康数据集上差距应该更大。

#### 收尾

`leg_clean.sh` 的 cleanup 这次执行了：`git status --porcelain` 为空，
`attention_v1.py`=`3ffdd3667b206531740ed46c69285f41`、`envs.py`=`348a3fbf05cd70fd37d80f9cb9a62b36`
（pristine），容器已停，8 张卡全部 `No running processes`。
