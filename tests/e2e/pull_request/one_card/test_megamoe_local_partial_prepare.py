...
def _assert_matches_reference(inputs, actual):
...
def test_local_partial_preparation_exact(tokens, probability_dtype, mask_dtype):
    inputs = _inputs(tokens, probability_dtype, mask_dtype)
    snapshots = tuple(None if x is None else x.clone() for x in inputs)
    actual = prepare_cann_megamoe_local_partial(*inputs, 256, 0, 1)
    _assert_matches_reference(inputs, actual)
    for source, snapshot in zip(inputs, snapshots):
        if source is not None:
            assert torch.equal(source, snapshot)


def test_local_partial_preparation_queued_outputs_own_storage():
    inputs = [_inputs(513, torch.bfloat16, torch.int8) for _ in range(8)]
    outputs = [prepare_cann_megamoe_local_partial(*values, 256, 0, 1) for values in inputs]
    torch.npu.synchronize()
    for values, actual in zip(inputs, outputs):
        _assert_matches_reference(values, actual)
    for index in range(4):
