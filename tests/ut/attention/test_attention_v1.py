...
class TestAttentionGraphHelpers(TestBase):
    ...
    def test_large_head_uses_fia_inside_full_decode_graph(self):
        # ATB paged attention rejects the ACL capture stream, so a 512-dim
        # global head must fall back to FIA when decode is captured full-graph.
        vllm_config = MagicMock()
        vllm_config.speculative_config = None
    ...
    def test_large_head_keeps_paged_attention_without_full_graph(self):
